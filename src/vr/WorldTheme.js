...
export class WorldTheme {
  ...
  update(delta, time, activity = 0) {
    ...
    this.particles.material.opacity = 0.25 + twinkle * 0.35 + Math.max(0, activity) * 0.4;
  }

  setParticleColor(hex) {
    if (this.particles?.material) this.particles.material.color.setHex(hex);
  }
  ...
  applyColorblindMode(mode) {
    ...
    if (!preset) return;
    this.setFogColor(remapColor(preset.fogColor, mode), preset.fogDensity);
    this.ambient.color.setHex(remapColor(preset.ambientColor, mode));
    this.setLightColor(remapColor(preset.pointColor, mode));
    this._setGridColors(remapColor(preset.gridColor1, mode), remapColor(preset.gridColor2, mode));
    this.setParticleColor(remapColor(preset.gridColor1, mode));
  }
