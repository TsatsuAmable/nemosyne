...
import { MovablePanel } from './ui/MovablePanel.js';
...
export class InputTelemetry extends MovablePanel {
  ...
  update(delta, time) {
    try {
      this._doUpdate(delta, time);
    } catch (err) {
      this.lines = [];
      this.log(`TELEMETRY ERROR: ${err?.message ?? err}`);
      this.render();
    }
  }

  _doUpdate(delta, time) {
   ...
   */
  _findHand(src, allSources, hands) {
    if (src?.handedness) {
   ...
   */
  _findController(src, allSources, controllers) {
    if (src?.handedness) {
