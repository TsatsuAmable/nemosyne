...
export class VaultPanel extends MovablePanel {
  ...
  renderContent(ctx: CanvasRenderingContext2D, w: number, contentH: number): void {
    ...
    ctx.strokeStyle = this.highContrast ? '#ffffff' : cssHex(PALETTE.panelBorder);
    ctx.lineWidth = 2;
    ctx.beginPath();
  ...
  private _renderLeftPane(ctx: CanvasRenderingContext2D, paneW: number, _h: number): void {
    const pad = 24;
    ctx.fillStyle = this.highContrast ? '#ffffff' : cssHex(PALETTE.panelText);
    ...
    pageArchives.forEach((archive, i) => {
      ...
      ctx.fillStyle = isSelected
        ? (this.highContrast ? '#333333' : 'rgba(0, 255, 204, 0.15)')
        : 'rgba(255, 255, 255, 0.03)';
      ctx.fillRect(pad, y, paneW - pad * 2, 70);

      ctx.strokeStyle = isSelected
        ? (this.highContrast ? '#ffffff' : '#00ffcc')
        : (this.highContrast ? '#444444' : cssHex(PALETTE.panelBorder));
      ...
      ctx.fillStyle = this.highContrast ? '#aaaaaa' : '#88aaff';
      ...
      ctx.fillText(`Events: ${archive.eventCount} | Discoveries: ${archive.discoveryCount}`, pad + 12, y + 58);
    });
...
ctx.fillStyle = this.highContrast ? '#ffffff' : cssHex(PALETTE.panelText);
      ...
      // Prev Button
      ctx.fillStyle = this.currentPage > 0 ? (this.highContrast ? '#ffffff' : '#00ffcc') : '#444444';
      ctx.strokeRect(pad, 375, 80, 32);
      ctx.font = this._scaleFont('bold 12px monospace');
      ctx.fillText('◀ PREV', pad + 18, 395);

      // Next Button
      ctx.fillStyle = this.currentPage < totalPages - 1 ? (this.highContrast ? '#ffffff' : '#00ffcc') : '#444444';
      ctx.strokeRect(paneW - pad - 80, 375, 80, 32);
      ctx.fillText('NEXT ▶', paneW - pad - 80 + 18, 395);
    }
  }

  private _renderRightPane(ctx: CanvasRenderingContext2D, paneX: number, totalW: number, _h: number): void {
    const pad = 24;
    ...
    if (this.showConfirmRestore) {
      ...
      ctx.strokeStyle = this.highContrast ? '#ffffff' : '#ff5577';
      ctx.strokeRect(startX, 280, btnW, 140);

      ctx.fillStyle = this.highContrast ? '#ffffff' : '#ff5577';
      ctx.font = this._scaleFont('bold 12px monospace');
  ...
  private _handleLeftPaneClick(cx: number, cy: number, paneW: number, pad: number): void {
    ...
    if (totalPages > 1) {
      // Prev Button
      if (this.currentPage > 0 && cx >= pad && cx <= pad + 80 && cy >= 375 && cy <= 375 + 32) {
        this.currentPage--;
        this.render();
        return;
      }
      // Next Button
      if (this.currentPage < totalPages - 1 && cx >= paneW - pad - 80 && cx <= paneW - pad && cy >= 375 && cy <= 375 + 32) {
        this.currentPage++;
        this.render();
        return;
      }
  ...
  private _handleRightPaneClick(cx: number, cy: number, paneX: number, pad: number): void {
    ...
    if (this.showConfirmRestore) {
      // Confirm Restore button inside overlay
      if (cx >= startX + 12 && cx <= startX + 112 && cy >= 360 && cy <= 360 + 32) {
        if (this.selectedArchiveId && this.onRestore) {
      ...
      if (cx >= startX + 130 && cx <= startX + 230 && cy >= 360 && cy <= 360 + 32) {
        this.showConfirmRestore = false;
        this.render();
        return;
      }
