...
export class MiniOverview {
  ...
  private _nodePos: THREE.Vector3;
  ...
  private _recomputeBounds(nodeMeshes: THREE.Mesh[], camera: THREE.Camera): void {
  ...
  private _worldToCanvas(x: number, z: number): { x: number; y: number } {
  ...
  private _draw(nodeMeshes: THREE.Mesh[], camera: THREE.Camera): void {
    const ctx = this.ctx;
