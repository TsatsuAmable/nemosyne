...
export class HolographicInspector {
   ...
   * @param {THREE.Object3D} nodeMesh
   * @param {object} data
   * @param {import('../Hands.js').HandPointer|import('../Controllers.js').ControllerPointer} pointer
   */
  showAtNode(nodeMesh, data, pointer, title = 'DATA NODE') {
    this.data = data;
    this.title = title;
    this.active = true;
  ...
  _updateTargetFromPointer() {
    ...
    if (pointer.getWorldPosition) {
      ...
      this._handPos.copy(pointer.rayOrigin);
    } else {
    ...
    if (pointer.getHandTransform) {
      ...
      this._handDir.copy(pointer.rayDirection);
    } else {
  ...
  _playCloseFeedback() {
    if (!this.engine?.input?.feedback) return;
    const fb = this.engine.input.feedback;
    fb.playTone({ frequency: 660, duration: 0.08, shape: 'sine', volume: fb.volume * 0.7 });
  }
