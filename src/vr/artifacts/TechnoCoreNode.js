 ...
 */
export class TechnoCoreNode {
  static LENS_MODES = ['off', 'statistical', 'anomaly'];

  constructor({ position = [6, 4, -8], scale = 1 } = {}) {
  ...
  setLensMode(mode) {
    ...
    this.lensMode = mode;
    const colorHex = this._modeColors[mode] ?? this._modeColors.off;
    const color = new THREE.Color(colorHex);
    this.coreMat.color.copy(color);
    this.sphereMat.color.copy(color);
    this.ring1.material.color.copy(color);
    this.ring2.material.color.copy(color);
    return true;
  }
  ...
  update(delta, time) {
    ...
    this.ring1.material.opacity = Math.min(1, ringPulse);
    this.ring2.material.opacity = Math.min(1, ringPulse);
  }
}
