...
import type { AudioToneOptions, LensMode, PointerLike } from '../coordinators/types.ts';

export interface SelectionFeedbackOptions {
  enabled?: boolean;
  volume?: number;
...
export class SelectionFeedback {
   ...
   */
  playCoreTone(mode: LensMode): void {
    const map: Record<LensMode, Omit<AudioToneOptions, 'volume'>> = {
  ...
  playPortalTone(zone: string, operation: string | null): void {
    ...
    const map: Record<string, Omit<AudioToneOptions, 'volume'>> = {
   ...
   */
  playGestureTone(gesture: string): void {
    const map: Record<string, Omit<AudioToneOptions, 'volume'>> = {
   ...
   */
  playHaptic(value = 0.6, durationMs = 40, source: HapticSource | null = null): void {
   ...
   */
  flashPointer(pointer: PointerLike, color = 0xffffff, durationMs = 120): void {
    if (!this.enabled || !this.visualEnabled) return;
    const ray = pointer?.ray as RayMesh | undefined;
   ...
   */
  showHitMarker(scene: THREE.Scene, position: THREE.Vector3, color = 0x00ffcc, durationMs = 180): void {
