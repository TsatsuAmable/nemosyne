...
export const GESTURE_MAP: Record<string, GestureMeta> = {
  pinchTogether: {
 ...
 */
export function gesturesForAction(actionLabel: string): string[] {
  return Object.entries(GESTURE_MAP)
    .filter(([, meta]) => meta.action === actionLabel)
    .map(([name]) => name);
}

export function getGestureMeta(name: string): GestureMeta | null {
