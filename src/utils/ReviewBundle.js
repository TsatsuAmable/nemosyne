/**
 * Analysis Review Bundle (ARB) builder.
 *
 * Produces a privacy-first, user-initiated export of telemetry, performance
 * violations, and optionally dataset/session metadata. The bundle is returned
 * as a plain JSON object so the caller can download it or let the user review
 * it before sharing. Nothing is transmitted automatically.
 *
 * Privacy levels:
 *   - telemetry-only: frame timings, errors, operation/gesture counts, active
 *     dataset name and topology.
 *   - metadata: above + column names/types, row count, session duration.
 *   - full-session: above + the full session snapshot (dataset rows, history).
 */

import { DEFAULT_ACCESSIBILITY } from '../vr/coordinators/types.ts';

/** @typedef {'telemetry-only' | 'metadata' | 'full-session'} PrivacyLevel */

const VALID_PRIVACY_LEVELS = /** @type {PrivacyLevel[]} */ ([
  'telemetry-only',
  'metadata',
  'full-session',
]);

/**
 * @typedef {object} TelemetryReport
 * @property {number} version
 * @property {number} timestamp
 * @property {boolean} enabled
 * @property {object} session
 * @property {number} session.durationSeconds
 * @property {string} session.datasetName
 * @property {string} session.datasetTopology
 * @property {object} frames
 * @property {number} frames.count
 * @property {number} frames.dropped
 * @property {number} frames.averageMs
 * @property {number} frames.lastMs
 * @property {object} frames.histogram
 * @property {object} operations
 * @property {object} gestures
 * @property {object} errors
 * @property {number} errors.count
 * @property {number} errors.warnings
 * @property {number} errors.unhandledRejections
 * @property {{message:string, time:number, isWarning?:boolean}|null} errors.last
 */

/**
 * @typedef {object} BundleOptions
 * @property {object} telemetryCollector
 * @property {() => TelemetryReport} telemetryCollector.getReport
 * @property {object} performanceBudget
 * @property {() => object[]} performanceBudget.getViolations
 * @property {string} [appVersion]
 * @property {PrivacyLevel} [privacyLevel='telemetry-only']
 * @property {string} [userNotes]
 * @property {object} [dataset] A Dataset-like object with name, columns[], rows[].
 * @property {string} [datasetTopology]
 * @property {number} [sessionDurationSeconds]
 * @property {object} [sessionSnapshot]
 */

/**
 * @typedef {object} AnalysisReviewBundle
 * @property {number} version
 * @property {number} generatedAt
 * @property {string} appVersion
 * @property {PrivacyLevel} privacyLevel
 * @property {TelemetryReport} telemetry
 * @property {object[]} performance
 * @property {object} [metadata]
 * @property {string} [metadata.datasetName]
 * @property {string} [metadata.datasetTopology]
 * @property {number} [metadata.rowCount]
 * @property {Array<{name:string, type:string}>} [metadata.columnSchema]
 * @property {number} [metadata.sessionDurationSeconds]
 * @property {Record<string, number>} [metadata.operations]
 * @property {Record<string, number>} [metadata.gestures]
 * @property {object} [session]
 * @property {string} [userNotes]
 */

/**
 * Build an Analysis Review Bundle from the supplied collectors and optional
 * metadata. The bundle is local-only; no network call is made.
 *
 * @param {BundleOptions} options
 * @returns {AnalysisReviewBundle}
 */
export function buildReviewBundle({
  telemetryCollector,
  performanceBudget,
  appVersion = '1.0.0-alpha.1',
  privacyLevel = 'telemetry-only',
  userNotes,
  dataset,
  datasetTopology,
  sessionDurationSeconds,
  sessionSnapshot,
}) {
  if (!VALID_PRIVACY_LEVELS.includes(privacyLevel)) {
    throw new Error(`Invalid privacyLevel: ${privacyLevel}`);
  }
  if (!telemetryCollector || typeof telemetryCollector.getReport !== 'function') {
    throw new Error('buildReviewBundle requires a telemetryCollector with getReport()');
  }
  if (!performanceBudget || typeof performanceBudget.getViolations !== 'function') {
    throw new Error('buildReviewBundle requires a performanceBudget with getViolations()');
  }

  const rawTelemetry = telemetryCollector.getReport();
  const telemetry = {
    ...rawTelemetry,
    errors: {
      ...rawTelemetry.errors,
      last: rawTelemetry.errors.last ? sanitizeError(rawTelemetry.errors.last) : null,
    },
  };

  /** @type {AnalysisReviewBundle} */
  const bundle = {
    version: 1,
    generatedAt: Date.now(),
    appVersion,
    privacyLevel,
    telemetry,
    performance: performanceBudget.getViolations(),
  };

  if (privacyLevel === 'metadata' || privacyLevel === 'full-session') {
    bundle.metadata = buildMetadata({
      telemetry,
      dataset,
      datasetTopology,
      sessionDurationSeconds,
    });
  }

  if (privacyLevel === 'full-session') {
    if (sessionSnapshot) {
      bundle.session = sessionSnapshot;
    }
  }

  if (userNotes !== undefined && userNotes !== null && String(userNotes).length > 0) {
    bundle.userNotes = String(userNotes);
  }

  return bundle;
}

/**
 * Strip potentially identifying or sensitive details from an error snapshot.
 * Keeps the message and warning flag, removes stack traces that may contain
 * local file paths or URLs.
 *
 * @param {{message:string, time:number, isWarning?:boolean}} error
 * @returns {{message:string, time:number, isWarning:boolean}}
 */
function sanitizeError(error) {
  const message = String(error?.message ?? '');
  // Drop stack traces or any multi-line message that likely contains paths.
  const safeMessage = message.split('\n')[0].slice(0, 500);
  return {
    message: safeMessage,
    time: Number(error?.time ?? Date.now()),
    isWarning: !!error?.isWarning,
  };
}

/**
 * @param {object} options
 * @param {TelemetryReport} options.telemetry
 * @param {object} [options.dataset]
 * @param {string} [options.datasetTopology]
 * @param {number} [options.sessionDurationSeconds]
 * @returns {NonNullable<AnalysisReviewBundle['metadata']>}
 */
function buildMetadata({ telemetry, dataset, datasetTopology, sessionDurationSeconds }) {
  /** @type {NonNullable<AnalysisReviewBundle['metadata']>} */
  const metadata = {
    datasetName: telemetry.session?.datasetName ?? dataset?.name ?? '-',
    datasetTopology: telemetry.session?.datasetTopology ?? datasetTopology ?? '-',
    rowCount: extractRowCount(dataset),
    columnSchema: extractColumnSchema(dataset),
    sessionDurationSeconds: sessionDurationSeconds ?? telemetry.session?.durationSeconds ?? 0,
    operations: { ...telemetry.operations },
    gestures: { ...telemetry.gestures },
  };
  return metadata;
}

/**
 * @param {object} [dataset]
 * @returns {number}
 */
function extractRowCount(dataset) {
  if (!dataset) return 0;
  if (typeof dataset.rowCount === 'number') return dataset.rowCount;
  if (Array.isArray(dataset.rows)) return dataset.rows.length;
  return 0;
}

/**
 * @param {object} [dataset]
 * @returns {Array<{name:string, type:string}>}
 */
function extractColumnSchema(dataset) {
  if (!dataset || !Array.isArray(dataset.columns)) return [];
  return dataset.columns.map((column) => {
    if (typeof column === 'string') return { name: column, type: 'UNKNOWN' };
    return {
      name: String(column?.name ?? ''),
      type: String(column?.type ?? 'UNKNOWN'),
    };
  });
}

/**
 * Format a bundle as an indented JSON string ready for download or review.
 *
 * @param {AnalysisReviewBundle} bundle
 * @returns {string}
 */
export function formatReviewBundle(bundle) {
  return JSON.stringify(bundle, null, 2);
}

// Re-export the default accessibility constant so callers can use it if they
// embed accessibility settings into a full-session snapshot.
export { DEFAULT_ACCESSIBILITY };
