/**
 * Draco Empirical Tuner & Adaptive Solver Loop (Sprint 26.2).
 *
 * Implements the evidence-informed adaptive recommender loop specified in:
 * - `docs/ROADMAP.md` §1827 (Evidence-informed Draco loop)
 * - `docs/Nemosyne_Definitive_Vision_and_Roadmap.md` §13 Gate 7 (Adaptive Research)
 *
 * Consumes empirical study trial outcomes (accuracy, duration, NASA-TLX workload)
 * and adjusts Draco layout utility weights and topology preference costs.
 */
...
export class DracoEmpiricalTuner {
  ...
  /**
   * Computes composite utility score for an empirical trial.
    ...
    for (const layout of candidateLayouts) {
      ...
      if (n === 0) {
        results.push({
          ...
          avgDurationMs: 0,
          avgWorkload: 0,
          rationale: 'No empirical study outcomes recorded for this layout; retaining baseline expert prior.',
        });
        continue;
      }
      ...
      // Positive utility (>0.65) reduces solver penalty, poor utility (<0.45) increases penalty
      let adjustment = 0;
