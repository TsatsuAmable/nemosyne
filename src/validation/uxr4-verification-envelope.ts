...
export function composeUxr4LaneEvidence(
  ...
  if (fingerprints.size > 1 || fingerprints.has(null)) invalidReasons.push('lane evidence lacks one exact shared device build fingerprint');
  if (evidenceClasses.size > 1) invalidReasons.push('lane evidence spans incompatible evidence classes');
  if (lanes.some((lane) => !lane.custodyValid)) invalidReasons.push('one or more lane custody records are invalid');

  const observations = lanes.flatMap((lane) => lane.observations);
  if (invalidReasons.length > 0) {
    const results = Object.fromEntries(UXR4_EVIDENCE_CLASSES.map((evidenceClass) => [evidenceClass, {
      evidenceClass, status: 'INVALID_RUN' as const, reasons: invalidReasons.slice(0, 32),
