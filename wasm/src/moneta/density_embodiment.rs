...
pub fn moneta_build_density_embodiment_v1(
    ...
    serialize_density_embodiment(handle, input_ptr, input_len)
        .map(|output| crate::write_bytes_out(output.as_bytes(), out_ptr, out_len))
        .unwrap_or(0)
}

#[wasm_bindgen]
pub fn moneta_prepare_density_embodiment_v1(handle: u32, input_ptr: u32, input_len: u32) -> u32 {
    crate::prepared_results::prepare(handle, || {
        serialize_density_embodiment(handle, input_ptr, input_len)
    })
}

fn serialize_density_embodiment(
    handle: u32,
    input_ptr: u32,
    input_len: u32,
) -> Option<String> {
    ...
    crate::prepared_results::record_computation(crate::prepared_results::DENSITY);
