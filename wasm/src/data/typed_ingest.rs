...
impl Registry {
    const fn new() -> Self { Self { slots: Vec::new(), free: Vec::new() } }
    ...
    fn remove(&mut self, handle: u32) {
        ...
        if let Some(slot) = self.slots.get_mut(idx) {
            if slot.take().is_some() { self.free.push(handle); }
        }
    }
}
...
pub fn data_load_typed_columns(ptr: u32, len: u32) -> u32 {
    ...
    match parse(bytes) {
        Ok(dataset) => REGISTRY.lock().expect("typed registry poisoned").insert(dataset),
        Err(_) => 0,
    }
}
...
pub fn typed_dataset_destroy(handle: u32) {
    REGISTRY.lock().expect("typed registry poisoned").remove(handle);
}

#[wasm_bindgen]
pub fn typed_dataset_row_count(handle: u32) -> u32 {
    REGISTRY.lock().expect("typed registry poisoned").get(handle).map(|d| d.row_count() as u32).unwrap_or(0)
}
...
mod tests {
    ...
    #[test]
    fn parses_primitive_and_categorical_buffers_without_rows() {
        ...
        assert_eq!(dataset.categorical_column(1).unwrap().dictionary, vec!["a", "b"]);
    }

    #[test]
    fn rejects_trailing_or_invalid_codes() {
        let mut bytes = MAGIC.to_vec();
        push_u32(&mut bytes, 1);
        push_u32(&mut bytes, 1);
        bytes.push(3);
