...
fn rank(mut matrix: Vec<Vec<f64>>) -> usize {
    ...
    for col in 0..cols {
        ...
        for row in 0..rows {
            ...
            for c in col..cols { matrix[row][c] -= factor * matrix[pivot_row][c]; }
        }
        pivot_row += 1;
...
mod tests {
    ...
    fn linear_dependence_has_rank_two() {
        let rows = (0..20).map(|i| { let x = i as f64; let y = (i * i) as f64; vec![x, y, x + y] }).collect::<Vec<_>>();
        let a = numeric_linear_rank_artifact("fp", vec!["x".into(), "y".into(), "sum".into()], &rows, "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        assert_eq!(a.effective_feature_count, Some(2));
    }

    #[test]
    fn zero_matrix_has_rank_zero() {
        let a = numeric_linear_rank_artifact("fp", vec!["a".into(), "b".into()], &[vec![0.0, 0.0], vec![0.0, 0.0]], "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        assert_eq!(a.status, EffectiveFeatureAuthorityStatus::Eligible);
    ...
    fn rank_is_invariant_to_uniform_rescaling() {
        ...
        assert_eq!(rank(base), 2); assert_eq!(rank(tiny), 2); assert_eq!(rank(huge), 2);
    }

    #[test]
    fn unsupported_claim_abstains() {
        let a = numeric_linear_rank_artifact("fp", vec!["a".into(), "b".into()], &[vec![0.2, 0.8], vec![0.4, 0.6]], "COMPOSITIONAL_EFFECTIVE_DIMENSION");
    ...
    fn malformed_or_empty_numeric_evidence_abstains() {
        let non_rectangular = numeric_linear_rank_artifact("fp", vec!["a".into(), "b".into()], &[vec![1.0, 2.0], vec![3.0]], "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        ...
        let non_finite = numeric_linear_rank_artifact("fp", vec!["a".into()], &[vec![f64::NAN]], "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        ...
        let empty_rows = numeric_linear_rank_artifact("fp", vec!["a".into()], &[], "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        assert_eq!(empty_rows.status, EffectiveFeatureAuthorityStatus::Abstain);
        assert!(empty_rows.refusal_reasons.contains(&"INSUFFICIENT_EVIDENCE".into()));
        let empty_features = numeric_linear_rank_artifact("fp", vec![], &[vec![]], "NUMERIC_LINEAR_RANK_DIAGNOSTIC");
        assert_eq!(empty_features.status, EffectiveFeatureAuthorityStatus::Abstain);
        assert!(empty_features.refusal_reasons.contains(&"INSUFFICIENT_EVIDENCE".into()));
    }
    ...
    fn digest_binds_assumptions() {
        ...
        let assumptions = ASSUMPTIONS.iter().map(|value| (*value).into()).collect::<Vec<String>>();
        let mut changed_assumptions = assumptions.clone(); changed_assumptions[1] = "different numerical tolerance contract".into();
