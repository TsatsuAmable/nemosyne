...
import { composeUxr4LaneEvidence, type Uxr4LaneEvidence, type Uxr4QualificationProfile } from '../src/validation/uxr4-verification-envelope.ts';
...
export function finalizeUxr4Cohort(options: {
  ...
  const lanes: Uxr4LaneEvidence[] = options.sessionLabels.map((label) => {
    ...
    if (!verified.ok) {
      return {
        sessionId: label, buildId: 'invalid', deviceBuildFingerprint: null,
        evidenceClass: 'governed-physical-validation', custodyValid: false, observations: [],
      };
    }
    const analysis = readJson(path.join(evidenceDir, 'analysis.json'));
    if (!isRecord(analysis) || !Array.isArray(analysis.uxr4Observations)) {
      return {
        sessionId: verified.custody.sessionId, buildId: verified.custody.buildId,
        deviceBuildFingerprint: verified.custody.deviceBuildFingerprint,
        evidenceClass: verified.custody.evidenceClass === 'clean-production-qualification'
          ? 'clean-production-qualification' : 'governed-physical-validation',
        custodyValid: false, observations: [],
      };
    }
    return {
