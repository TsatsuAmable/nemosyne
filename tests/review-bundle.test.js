...
import { buildReviewBundle, formatReviewBundle, DEFAULT_ACCESSIBILITY } from '../src/utils/ReviewBundle.js';

function makeCollector(report = {}) {
  return {
    getReport: () => ({
      version: 1,
      timestamp: Date.now(),
...
function makeBudget(violations = []) {
  return {
...
describe('buildReviewBundle', () => {
  it('builds a telemetry-only bundle', () => {
    ...
    const budget = makeBudget([{ id: 'frameMs', value: 20 }]);
