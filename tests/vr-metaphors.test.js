...
function makeGraphDataset(count) {
  ...
  return { dataset: new Dataset('Graph', [{ name: 'id', type: ColumnType.CATEGORICAL }, { name: 'value', type: ColumnType.NUMERIC }], rows), edges };
}
...
describe('Interaction metaphors', () => {
  ...
  it('synthesises a beacon artefact for geo topology', () => {
    ...
    const artifact = VRTopologyTranslator.synthesizeArtifact(result, { topology: 'GEO', dataset: ds });
  ...
  it('applies a resonance pulse effect to graph neighbours', () => {
    ...
    const neighbor = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), new THREE.MeshBasicMaterial());
