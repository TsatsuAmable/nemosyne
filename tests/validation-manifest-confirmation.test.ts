...
describe('validation sink manifest confirmation', () => {
  ...
  it('rejects same-session build, lane, worktree and ADB identity drift', () => {
    ...
    expect(validationManifestConfirmationIssue(context(), manifest({ mode: 'quest-10m' }))).toMatch(
      /lane/i
    );
    expect(
      validationManifestConfirmationIssue(
        context(),
        manifest({ profileOverride: 'uxr0-resource-trend-30m' })
      )
    ).toMatch(/profile/i);
    expect(validationManifestConfirmationIssue(context(), manifest({ worktree: 'dirty' }))).toMatch(
      /worktree/i
    );
    expect(
      validationManifestConfirmationIssue(
