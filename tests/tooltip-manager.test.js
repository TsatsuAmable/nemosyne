...
import { TooltipManager } from '../src/vr/ui/TooltipManager.js';

function makeCamera(at = new THREE.Vector3(0, 1.6, 0), direction = new THREE.Vector3(0, 0, -1)) {
...
describe('TooltipManager', () => {
  it('hides all tooltips by default', () => {
    ...
    for (const entry of manager.pool) {
      ...
      expect(entry.mesh.material.opacity).toBe(0);
    }
  });
  ...
  it('renders tooltip content from the target row', () => {
    ...
    expect(active.mesh.material.map.version).toBeGreaterThan(0);
  });
