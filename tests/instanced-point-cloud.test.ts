...
describe('InstancedPointCloud', () => {
  ...
  it('updates colors and scales of individual instances', () => {
    ...
    expect((cloud as unknown as { _colors: Float32Array })._colors[0]).toBe(0);
    expect((cloud as unknown as { _colors: Float32Array })._colors[1]).toBe(0);
    expect((cloud as unknown as { _colors: Float32Array })._colors[2]).toBe(1);
    expect((cloud as unknown as { _scales: Float32Array })._scales[0]).toBe(2);
    expect(cloud.mesh.instanceColor).toBeTruthy();
  });
