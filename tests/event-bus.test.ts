...
describe('WorldEventBus', () => {
  ...
  it('rejects invalid topics', () => {
    expect(() => bus.on('' as any, () => {})).toThrow();
    expect(() => bus.on(123 as any, () => {})).toThrow();
    expect(() => bus.emit('' as any, {})).toThrow();
  });

  it('rejects non-function handlers', () => {
    expect(() => bus.on(WorldTopics.OPERATION_APPLIED, null as any)).toThrow();
    expect(() => bus.on(WorldTopics.OPERATION_APPLIED, 'not-a-function' as any)).toThrow();
  });
});
