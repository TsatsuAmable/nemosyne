...
function envelope(overrides: Partial<GovernedEventEnvelopeV1['runtime']['components']> = {}): GovernedEventEnvelopeV1 {
  ...
  const content: Omit<GovernedEventEnvelopeV1, 'contentDigest'> = {
    schemaVersion: '1', eventFamilyId: PRODUCT_OPERATION_FAMILY_ID, payloadSchemaVersion: '1',
    eventId: '22222222-2222-4222-8222-222222222222', streamId: 'strv1_44444444-4444-4444-8444-444444444444',
    producerInstanceId: 'piv1_33333333-3333-4333-8333-333333333333', streamSequence: 0,
    capturedAt: '2026-09-03T05:00:00.000Z', sourceComponent: PRODUCT_OPERATION_SOURCE_COMPONENT, mode: 'PRODUCT',
    purpose: GOVERNED_PURPOSES.PRODUCT_ANALYTICS, dataClasses: [GOVERNED_DATA_CLASSES.PRODUCT_INTERACTION_METADATA], effectiveSensitivity: 'PSEUDONYMOUS',
    identities: { profilePseudonymId: 'ppv1_k1_21c135cf2ec5ade8d7d9483d69ca18a5a59b7475fe8ba4576055f890fa1b65dc', productSessionId: 'psv1_55555555-5555-4555-8555-555555555555', investigationId: null, discoveryEpisodeId: null },
    dataset: null,
    runtime: { schemaVersion: '1', components: { applicationBuild: APP, deploymentConfiguration: DEPLOYMENT, wasmKernel: null, representationTreatment: null, monetaEngine: null, fitnessModel: null, nil: null, perceptionGestureTreatment: null, uiTreatment: UI, platformRuntime: PLATFORM, ...overrides }, randomSeeds: {} },
    authorization: [{ schemaVersion: '1', basis: 'CONSENT_RECEIPT', purpose: GOVERNED_PURPOSES.PRODUCT_ANALYTICS, authority: PRODUCT_ANALYTICS_DATA_SERVICE_AUTHORITY_REFERENCE, evidence: { id: 'receipt-1', revision: '1', digest: { algorithm: 'SHA256', value: 'e'.repeat(64) } }, policy: PRODUCT_ANALYTICS_OPERATION_NOTICE_REFERENCE }],
    retention: { schemaVersion: '1', policy: PRODUCT_ANALYTICS_OPERATION_RETENTION_REFERENCE }, payload,
...
function authority() {
  return new ReviewedProductAnalyticsRuntimeAuthority({ schemaVersion: '1', applicationBuild: APP, deploymentConfiguration: DEPLOYMENT, uiTreatment: UI, allowedPlatformRuntimes: [{ componentId: PLATFORM.componentId, version: PLATFORM.version }] });
}
...
describe('PT4B8 reviewed deployment runtime authority', () => {
  ...
  it('refuses runtime mismatch before the delegated persistence authority', async () => {
    ...
    const consent = new SqliteProductAnalyticsConsentAuthority({ dataDirectory, purposePseudonymKey: { version: 'p1', key: new Uint8Array(32).fill(7) }, deletionHandleKey, now });
    const delegate = new SqliteProductAnalyticsEventIngestion({ dataDirectory, deletionHandleKey, now });
    const ingestion = new RuntimePinnedProductAnalyticsEventIngestion({ delegate, runtimeAuthority: authority() });
    const principal = { issuer: 'https://issuer.example', subject: 'subject-123' };

    expect(await ingestion.ingestLine(principal, JSON.stringify(envelope({ applicationBuild: { ...APP, version: 'tampered' } })))).toMatchObject({ status: 'REFUSED_GOVERNANCE', reasonCode: 'RUNTIME_MANIFEST_MISMATCH' });
    expect(await ingestion.ingestLine(principal, JSON.stringify(envelope()))).toMatchObject({ status: 'REFUSED_GOVERNANCE', reasonCode: 'CONSENT_REQUIRED' });
    delegate.close();
    consent.close();
  });
});
