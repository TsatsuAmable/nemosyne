...
import { PerformanceBudget, DEFAULT_BUDGETS } from '../src/utils/PerformanceBudget.js';
...
describe('PerformanceBudget', () => {
  ...
  it('flags dropped frames over a 10-second window', () => {
    const now = 1000;
    for (let i = 0; i < 6; i++) {
      budget._recordDrop(true, now + i * 1000);
    }
...
describe('World performance integration', () => {
  ...
  it('toggles strict budget from settings', () => {
    ...
    expect(world.engine.performanceBudget.budgets.frameMs).toBe(13.33);
  });
  ...
  it('logs performance violations from engine tick', () => {
    ...
    world.engine.renderer.info = originalInfo;

    expect(warnSpy).toHaveBeenCalled();
    const calls = warnSpy.mock.calls.map((c) => c[0]);
    expect(calls.some((msg) => msg.includes('[PerformanceBudget]'))).toBe(true);
    warnSpy.mockRestore();
