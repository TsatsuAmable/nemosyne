...
import { PeerPresenceHUD } from '../src/vr/ui/PeerPresenceHUD.js';

function makeCameraGroup(x = 0, z = 0) {
...
describe('PeerPresenceHUD', () => {
  ...
  it('creates a canvas-texture mesh attached to the follow anchor', () => {
    ...
    expect(hud.mesh.material.map).toBeInstanceOf(THREE.CanvasTexture);
    expect(hud.mesh.parent).toBe(cameraGroup);
  });

  it('draws peer names when peers are present', () => {
    hud = new PeerPresenceHUD(cameraGroup, {
      getPeers: () => [{ peerId: 'p1', name: 'Alice', state: {} }],
      getLocalPeerId: () => null,
    });
  ...
  it('marks the local peer as "You"', () => {
    hud = new PeerPresenceHUD(cameraGroup, {
      getPeers: () => [{ peerId: 'me', name: 'Analyst', state: {} }],
      getLocalPeerId: () => 'me',
    });
  ...
  it('redraws when peer positions change', () => {
    const peers = [{ peerId: 'p1', name: 'Alice', state: { position: { x: 1, y: 1.6, z: -2 } } }];
    hud = new PeerPresenceHUD(cameraGroup, {
      getPeers: () => peers,
      getLocalPeerId: () => null,
    });
