...
describe('Tier 2 — Feature 12: Binary Protocol Bounds Safety (Boundary Cases)', () => {
  ...
  it('F12-BC3: ArrowBinaryParser.parseBinaryFloatStream with buffer < 8 bytes returns empty Dataset', () => {
    const buffer = new ArrayBuffer(4);
    const dataset = ArrowBinaryParser.parseBinaryFloatStream(buffer);
  ...
  it('F12-BC4: ArrowBinaryParser.extractZeroCopyPositions with truncated buffer returns empty Float32Array', () => {
    const buffer = new ArrayBuffer(4);
    const positions = ArrowBinaryParser.extractZeroCopyPositions(buffer);
