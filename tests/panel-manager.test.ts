...
class TestPanel extends MovablePanel {
  constructor(cameraGroup: THREE.Group, title: string, position: [number, number, number]) {
    super(cameraGroup, {
      title,
      width: 400,
      height: 300,
...
function makeRaycasterForPoint(point: THREE.Vector3): THREE.Raycaster {
...
describe('PanelManager free-floating mode', () => {
  ...
  it('serializes and restores panel positions', () => {
    ...
    const restoredA = restored.panels.find((p) => p.title === 'A') as MovablePanel;
    const restoredB = restored.panels.find((p) => p.title === 'B') as MovablePanel;
