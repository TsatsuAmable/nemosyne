...
import {
  adjudicateUxr4Envelope,
  UXR4_EVIDENCE_CLASSES,
  UXR4_QUALIFICATION_PROFILES,
} from '../src/validation/uxr4-verification-envelope';
describe('UXR4 verification envelope', () => {
  it('freezes governed profiles and evidence classes', () => {
    expect(UXR4_QUALIFICATION_PROFILES).toEqual([
      'functional-5m',
      'resource-trend-30m',
      'sustained-60m',
      'scale-staircase',
    ]);
    expect(UXR4_EVIDENCE_CLASSES).toEqual([
      'interaction',
      'responsiveness',
      'frame-render',
      'memory-resource',
  ...
  it('keeps failure classes independent', () => {
    const observations = UXR4_EVIDENCE_CLASSES.map((evidenceClass) => ({
      evidenceClass,
      status: evidenceClass === 'memory-resource' ? ('FAIL' as const) : ('PASS' as const),
      reasons: [evidenceClass],
    }));
    const result = adjudicateUxr4Envelope({
      schemaVersion: 1,
      profile: 'resource-trend-30m',
    ...
    expect(result.results['memory-resource'].status).toBe('FAIL');
  });
  it('fails closed when a governed evidence class is absent', () => {
    const observations = UXR4_EVIDENCE_CLASSES.filter((x) => x !== 'semantic-scale').map(
      (evidenceClass) => ({
        evidenceClass,
        status: 'PARTIAL' as const,
        reasons: ['captured without a governed automatic threshold'],
      })
    );
    const result = adjudicateUxr4Envelope({
      schemaVersion: 1,
      profile: 'functional-5m',
      evidenceClass: 'governed-physical-validation',
      observations,
    });
...
describe('UXR4 QV4 lane integration', () => {
  it('maps perf gates only to frame/render and memory/resource', () => {
    expect(
      deriveUxr4LaneObservations(laneManifest('quest-perf'), [
      ...
      {
        evidenceClass: 'memory-resource',
        status: 'PARTIAL',
        reasons: ['PERF-05: no invented threshold'],
      },
    ]);
  });
  ...
  it('does not manufacture absent cross-lane observations', () => {
    expect(
      deriveUxr4LaneObservations(laneManifest('quest-10m'), [
        { gate: 'RF-029', status: 'PASS', reasons: ['bounded'] },
        { gate: 'RF-051', status: 'PASS', reasons: ['bounded'] },
      ])
    ).toEqual([
      {
        evidenceClass: 'semantic-scale',
        status: 'PASS',
        reasons: ['RF-029: bounded', 'RF-051: bounded'],
      },
    ]);
  });
});
...
const exactLane = (
  ...
  sessionId,
  buildId: 'build-a',
  deviceBuildFingerprint: 'device-a',
  evidenceClass: 'governed-physical-validation' as const,
  sourceProfile: null,
  custodyValid: true,
  observations,
  ...overrides,
...
describe('UXR4 cross-session composition', () => {
  const obs = (evidenceClass: any, status: any = 'PASS') => ({
    evidenceClass,
    status,
    reasons: [evidenceClass],
  });
  it('composes exact-build exact-device lanes without averaging class status', () => {
    const result = composeUxr4LaneEvidence('scale-staircase', [
      ...
      exactLane('perf', [obs('frame-render'), obs('memory-resource', 'PARTIAL')], {
        sourceProfile: 'quest-3s-qualification',
      }),
  ...
  it('rejects relabelling a shorter performance lane as a stronger UXR4 profile', () => {
    const result = composeUxr4LaneEvidence('resource-trend-30m', [
      exactLane('ux', [obs('interaction'), obs('responsiveness')]),
      exactLane('perf', [obs('frame-render'), obs('memory-resource', 'PARTIAL')], {
        sourceProfile: 'uxr0-functional-5m',
      }),
      exactLane('scale', [obs('semantic-scale')]),
    ]);
    expect(result.aggregateStatus).toBe('INVALID_RUN');
    expect(result.results['frame-render'].reasons.join(' ')).toMatch(/profile does not match/i);
  });
  it('fails closed on build or device mismatch', () => {
    const result = composeUxr4LaneEvidence('scale-staircase', [
      exactLane('ux', [obs('interaction'), obs('responsiveness')]),
      exactLane('perf', [obs('frame-render'), obs('memory-resource')], {
        buildId: 'build-b',
        sourceProfile: 'quest-3s-qualification',
      }),
    ...
    expect(result.results.interaction.reasons.join(' ')).toContain(
      'shared device build fingerprint'
    );
  });
  it('fails closed on invalid custody', () => {
    const result = composeUxr4LaneEvidence('functional-5m', [
      exactLane('ux', [obs('interaction')], { custodyValid: false }),
    ]);
    expect(result.aggregateStatus).toBe('INVALID_RUN');
...
describe('UXR4 cohort finalizer', () => {
  it('fails closed when supplied sessions are not finalized custody bundles', () => {
    ...
    const artifact = finalizeUxr4Cohort({
      validationLogRoot: root,
      profile: 'functional-5m',
      sessionLabels: ['not-finalized'],
    });
