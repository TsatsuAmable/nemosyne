...
import { MiniOverview } from '../src/vr/ui/MiniOverview.ts';

function makeMesh(name = 'node', x = 0, z = 0): THREE.Mesh {
...
function makeCamera(x = 0, z = 0): THREE.PerspectiveCamera {
...
describe('MiniOverview', () => {
  ...
  afterEach(() => {
    if (overview?.mesh) {
      const mat = overview.mesh.material as THREE.MeshBasicMaterial;
      mat.map?.dispose?.();
      mat.dispose?.();
  ...
  it('creates a canvas-texture mesh attached to the follow anchor', () => {
    ...
    expect((overview.mesh.material as THREE.MeshBasicMaterial).map).toBeInstanceOf(THREE.CanvasTexture);
