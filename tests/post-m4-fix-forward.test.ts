...
describe('post-M4 independent fix-forward', () => {
  it('keeps semantic status copy bound to the candidate family', () => {
    ...
    expect(nodeSource).toContain("'UNAVAILABLE',\n            undefined,\n            candidateId");
  });

  it('publishes a dataset-loaded event only for a fresh authoritative dataset transition', () => {
    ...
    const authority: DatasetLoadAuthority = {
