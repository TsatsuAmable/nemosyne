...
**Current remote main at roadmap branch cut:** `600bd43` (#507 merged). #503 merged the P1-U6 vault/portals + P1-USIM simulator substrate and P1-U7/U8 convergence; #504 landed the P1-Q Q3C same-operation materialization falsifier; #505 fixed RF-059 row-identity remap linearization; #506 landed the P1-Q Q8 supply-chain prevention pilot; #507 landed the Q3D browser operation envelope decomposition. #485/#486 landed RF-035B1 reference-backed history/version state and branch-point materialisation; #487 landed RF-035B2A compact authoritative row-view transfer for verified edge-free `filter`/`sort`/`slice`; #488 landed RF-035B2B reference-backed live durable result/event storage with isolated per-lineage row values and lazy schema-v2 materialisation; #489/#490 planned IWER as the simulator tier plus the USIM-A architecture-conformance and gated USIM-C collaboration packs. RF-035 remains **IMPLEMENTATION PARTIAL / REVIEW ACTIVE** because graph/derived Worker results, session/package materialisation, handle-only/typed state and measured whole-pipeline browser/WASM/device evidence remain. The next Stream-B scale tranche is real browser module-Worker + real-WASM transfer/heap/GC measurement, not another unmeasured memory rewrite. P1-Q is a parallel-safe engineering-quality/cadence substrate: Q0 architecture policy is promoted to required boundaries, Q1 classified nextest/sccache (both rejected for required lanes), Q2 landed fast-check adoption with cross-language goldens and classified proptest targeted-only, Q3 landed failure-evidence/resource-envelope/materialization pilots, Q8 landed a non-required supply-chain pilot (cargo-deny + Dependency Review), and Q9 landed the exact-head promotion controller + RF-052 ruleset truth reconciliation (live `Protect main` ruleset now requires `approval-gate`; `governance/promotion-policy.json` and `npm run governance:policy` hold it mechanically). Mutation/fuzz/Miri/formal/network-chaos work remains targeted or scheduled unless an owning RF requires it. #478's title did **not** implement P1-U6; P1-U6 remains partial. Static resource limits remain kernel safety guards, not Quest qualification and not evidence of generic 10M-row support.
...
| RF-052 | Engineering governance / merge evidence | Medium | The active main ruleset is named as an approval gate but currently requires zero approving reviews; the separate Continuous Copilot Review ruleset is disabled. CI is meaningful and green, but branch-rule wording/behavior can imply independent review that is not actually enforced. | **IMPLEMENTATION LANDED / REVIEW ACTIVE via Q9:** the live `Protect main` ruleset now requires `approval-gate` as a required status check (name now matches enforcement), while `required_approving_review_count` remains `0` because the approval authority is the `approval-gate` workflow (owner approval on the exact PR head), not a GitHub approving review. `governance/promotion-policy.json` declares the intended policy, `npm run governance:policy` fails CI when the live ruleset drifts, and the non-required P1-Q Q9 promotion controller verifies exact-head evidence (required checks green on the exact head, clean review threads, adversarial disposition present) and revokes on any head movement without manufacturing an approval. Keep review-thread resolution enforced; keep Node 24 aggregate CI + CodeQL as engineering gates while distinguishing them from scientific/UX/device verification. Periodic ruleset/config drift check is wired via the governance workflow. |
...
| RF-059 | P1-A/P1-B / Rust row identity & scale | High | Q3C same-operation measurement exposed worst-case `O(n² × columns)` source-row identity recovery in `Dataset::clone_with_rows`: every row-preserving output rediscovered its source through a linear equality scan, dominating Rust `sort` at 32k rows. | **IMPLEMENTATION LANDED / REVIEW ACTIVE via #505:** index declared scientific row values once and consume FIFO source indices so duplicate, missing/null, signed-zero, NaN, durable-row-ID and graph-remap semantics remain equivalent. Same-harness Q3C remeasurement reduced the 32k Rust sort kernel from ~36.4 s to ~0.43 s (~83–84x; ~98.8% reduction) with identical authoritative scientific fingerprints. Preserve the regressions and keep RF-029/RF-035/RF-051 open for the remaining full pipeline/device envelope. |
...
5. **P1-USIM + AR-7 RF-049 + AR-8 RF-050** belong to the parallel XR convergence/evidence stream: **USIM-0 (simulator adapter + evidence boundary) is implementation-landed/review-active**; IWER supplies both interaction evidence and the USIM-A architecture-conformance tier for lifecycle, persistence, reference-space and clean-artifact invariants; physical Quest remains the promotion authority for device-dependent evidence.
...
- [/] pilot GitHub Dependency Review and `cargo-deny` on the shipped Rust/WASM graph (bans/licences/sources policy landed in `wasm/deny.toml`; non-required pilot workflow wired); classify as required only after measured PR-time signal and wall-clock cost;
...
- [/] define one promotion authority that requires the exact current SHA to have required checks green, no unresolved material threads, required adversarial/post-review evidence and truthful roadmap/status state (`scripts/check-promotion-evidence.mjs` + non-required P1-Q Q9 promotion workflow; promotion to a required gate awaits measured signal);
- [x] revoke promotion on any head movement; coordinate terminology/behavior with RF-052 so automation never implies an approval that did not occur (controller reports evidence only; live ruleset now requires `approval-gate` so its name matches enforcement; `governance/promotion-policy.json` + `npm run governance:policy` drift check wired).
...
#### P1-USIM — WebXR simulator substrate and golden spatial scenarios — PLANNED ENABLER (USIM-0 LANDED)
...
**USIM-0 — simulator adapter and evidence boundary — IMPLEMENTATION LANDED / REVIEW ACTIVE**

- [x] add a dev/test-only IWER adapter; prove no simulator/dev-UI dependency is reachable from the production bundle;
- [x] route simulated controller/hand/head input through the real WebXR -> InputRouter path, never directly to NIL/Atlas or component callbacks;
- [x] map the useful `WebXR6DoFPoseRig` presets into scenario fixtures rather than maintaining a separate mock WebXR runtime;
- [x] retain `SpatialErgonomicsLinter` as the measurement layer and connect simulator runs to bounded `XREvaluationEpisode` evidence (`environment.mode = desktop-simulator`);
- [x] fail unsupported simulator capabilities explicitly rather than fabricating success.

**USIM-0 evidence (implementation landed, review active):** `dev/xr-simulator/` provides `WebXRSimulatorAdapter` (IWER install/uninstall with DOM-shim restoration, real `navigator.xr` immersive-vr session, head/controller/hand pose + trigger/pinch driving, capability fail-closed), `XREvaluationEpisode` bounded recorder (`environment.mode = desktop-simulator`), `ScenarioFixtures` mapping `WebXR6DoFPoseRig.PRESETS`, and `SimulatorScenarioRunner` that drives the real production `InputRouter` through real IWER `XRFrame`/`XRInputSource`/`XRReferenceSpace`, binding input sources through the production `connected` handler exactly as three.js WebXRManager does. `tests/ui-system/webxr-simulator.test.ts` (13 tests) proves a real session starts, a production control is activated through the real InputRouter with controller simulation and one supported hand path (both driven through real XR objects, not direct callbacks), deterministic scenario replay emits bounded `desktop-simulator` episode evidence, `SpatialErgonomicsLinter` evaluates the simulated pose against a rendered target, disabling restores ordinary WebXR behavior, the shared IWER hand-pose config is restored on uninstall so no geometry mutation leaks into other devices, `src/` has no `iwer`/`dev/` import path and `iwer` is a devDependency only, and unsupported capabilities fail closed. Remaining USIM-0 residual: the hand-pinch geometry must be rewritten deterministically because IWER's shipped pinch pose (~9cm tip gap) exceeds Nemosyne's 0.04m threshold — this is recorded as an IWER capability limitation, and physical Quest hand evidence stays authoritative under U1/U9.
...
**USIM-A evidence (lifecycle / async race landed, review active):** `dev/xr-simulator/LifecycleScenarioRunner.ts` drives the real production `InputRouter` through real IWER `XRFrame`/`XRInputSource`/`XRReferenceSpace` while an async analysis is in flight on the real `DerivedAnalysisScheduler` (the RF-061 generation guard keyed on dataset version + fingerprint), injects real lifecycle faults (`WebXRSimulatorAdapter.setSessionVisibilityState` -> hidden, `setInputSourceConnected` -> false, `endSession`), completes the analysis after session exit, re-enters XR, and asserts (a) exactly one committed analysis survives the exit/re-enter under the same generation (no duplicate, no discard), (b) a stale-generation completion is rejected by the real `isCurrent` guard and never published, and (c) a stale captured trigger press is not replayed as a selection in the new session while a fresh press still commits exactly once. `tests/ui-system/usim-a-lifecycle.test.ts` (3 tests) also proves the fault-injection levers themselves (visibility hidden empties `session.inputSources` and fires `visibilitychange`; input-source disconnect removes the source). This is deterministic `desktop-simulator` conformance evidence through the real router/scheduler/session surfaces; it is not device qualification and does not substitute for physical Quest U1/U8/U9 or PERF-04 evidence. Remaining USIM-A items (presentation-independent reproducibility, reference-space integrity, resource lifecycle balance, clean-production immersive boot, Moneta presentation independence) remain open.
