...
| CI-02    | Security             | High     | CodeQL was excluded from pull requests by a stale migration exception.                                                                                                                                                                                       | **Fixed:** PR analysis restored.                                                                                                           |
...
| ARCH-01  | Architecture         | High     | `World.ts` is 2,178 lines; `RuntimeBridge.ts` 1,285; coordinator types 1,237; `AtlasCore.ts` 1,022; and `VRTopologyTranslator.ts` 1,012. Existing “god-object refactoring” tests prove class existence, not cohesion or dependency direction.                | **Open P1:** split by lifecycle/authority seam with characterization tests; no cosmetic file slicing.                                      |
| ARCH-02  | Lifecycle            | High     | `WorldUIManager` constructs many GPU/UI resources but has no explicit aggregate disposal contract. Engine-wide teardown masks ownership mistakes and makes world recreation/partial recovery risky.                                                          | **Open P1:** introduce idempotent ownership/disposal tests and world recreation soak.                                                      |
| PERF-03  | Spatial queries      | High     | `three-mesh-bvh` and a custom `SpatialIndex` exist but are not used by production selection. Dynamic ray paths remain linear in registered meshes.                                                                                                           | **Open P1:** benchmark primitive count/device crossover, wire one authoritative accelerator and delete the redundant built-only path.      |
...
| RES-01   | Kernel recovery      | High     | `KernelUnavailable` is explicit, but broader panic/handle invalidation/world-recreation recovery has not been exercised as a sustained fault campaign.                                                                                                       | **Open P1:** add injected failures at ABI calls and verify bounded cleanup plus user recovery.                                             |
...
1. Extract World boot/recovery and runtime teardown into an idempotent lifecycle owner.
2. Split `RuntimeBridge` by ABI handle family while keeping one readiness/authority state machine.
