...
| ARCH-01  | Architecture         | High     | `World.ts` is 2,178 lines; the pre-split `RuntimeBridge.ts` was 1,318; coordinator types 1,237; `AtlasCore.ts` 1,022; and `VRTopologyTranslator.ts` 1,012. Existing “god-object refactoring” tests proved class existence, not cohesion or dependency direction. | **In progress:** lifecycle ownership is merged; `refactor/runtimebridge-abi-handle-families` makes RuntimeBridge a frozen compatibility facade over one generation-fenced state owner and typed ABI families. Coordinator, Atlas and topology-embodiment decomposition remain open. |
