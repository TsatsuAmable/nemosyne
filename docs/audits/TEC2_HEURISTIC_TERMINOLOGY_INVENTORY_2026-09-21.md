...
- the Rust producer's non-emission of a topology record conflates not-applicable states (no temporal statistics entry, no coordinate pair) with not-declared or intentionally-absent states (hierarchy is unconditionally `None`, edges are `None` or empty), so it is not a topology classification. (A spectral sampling gap is **not** among them: when a temporal statistics entry exists the producer still emits `Some(TemporalProfile)` with degraded seasonality, so that case reaches the presence branch.)
...
- **The `TABULAR` compatibility value and `inferTopology` are unchanged.** The TEC3 addendum established that no production code reads the epistemic source of `topologicalStructure.topology`, and in `FitnessModel.scoreStructure` the value is used only to *grant* boosts, never to deny them — the absent-structure case "simply fails to earn a boost". (Outside `FitnessModel` the value also appears in hard-constraint conditions in `MonetaHypothesisEngine` and `ContextualTaskSurface`, where `TABULAR` behaves identically to absence and none of those sites reads the epistemic source.) The mislabel was a latent integrity defect, not a demonstrated decision error, so a value or decision-behavior change was not warranted by evidence.
...
- **`cardinality.edgeCount` graph-absence marking is not the same defect and is unchanged.** `analyze_graph` (`wasm/src/data/profile.rs:610-613`) returns `None` iff the dataset's declared edges are empty, and both upstream states (edges absent vs `Some(empty)`) yield the same true statement — zero declared edges — so `edgeCount: 0` marked `derived` is honest. The topology absence, by contrast, spans four families whose non-emission carries materially different meanings.
