...
**Current remote main at the start of this Stream-B tranche:** `7caf490a` (#429, `fix(scale): preflight clustering resource budgets`). #429 landed the first RF-029/RF-031 resource-envelope cut: Rust-owned `AnalysisBudget`/`ResourceEstimate`, saturating work arithmetic, dense-CSR transient accounting and canonical operation-ABI preflight for K-means, hierarchical clustering and DBSCAN. Those static limits are kernel safety guards, not Quest qualification and not evidence of generic 10M-row support.

**Current Stream-B tranche:** `fix/rf030-tda-resource-envelope` extends the merged #429 envelope onto the production TDA path. Rust now owns a `data_tda_resource_preflight` ABI that reads the resident columnar validity buffers, counts complete-case eligible observations without manufacturing missing values as zero, and estimates Mapper, persistence and Betti-0 work/memory before the canonical TypeScript bridge enters the raw TDA export. Mapper `bins`, Betti `steps`, worst-case dense CSR, and persistence/Betti duplicate edge buffers are included. Above budget the production bridge throws a typed `UnsupportedAtScaleError` carrying the Rust-authored reason and eligibility/resource metadata rather than returning a plausible empty topology result. A real-WASM 9,000×7 boundary test exercises the high-dimensional exact-fallback refusal. This tranche is **IMPLEMENTATION PARTIAL / REVIEW ACTIVE**, not verified complete.

**Adversarial review note for the current cut:** review found three scale-control seams before PR creation. First, Mapper `bins` and Betti `steps` are user-controlled multipliers and had to be part of the resource estimate. Second, a feature-selection seam that could not borrow primitive validity initially returned no preflight, which would have allowed large work to bypass the guard; it now budgets all source rows conservatively and records `conservative_source_rows_preflight`. Third, repository-wide caller review found production `src/` TDA calls are centralized through `DatasetHandleBridge`, but the low-level raw WASM `data_compute_*` exports remain callable directly and are not themselves guarded by the sibling preflight. RF-030 therefore remains review-active until the kernel boundary itself cannot bypass the envelope, resource refusals gain durable provenance/NIL semantics, and any approximation mode is explicit and quality-governed.

**Reprioritised Stream-B critical path:** (1) RF-007/RF-017 validity correctness landed in #426, with provenance/scale residuals; (2) RF-028/RF-032/RF-036 temporal/topology evidence work remains unmerged and review-active; (3) **CURRENT:** RF-029/RF-030/RF-031/RF-035 analytical resource envelope, with #429 merged and RF-030 production TDA guard in review; (4) RF-001/RF-002 representation truth; (5) RF-005/RF-006/RF-008 plus the RF-027 World/UX carry-over; (6) RF-015/RF-033 production evidence; (7) physical Quest 3S qualification; (8) private-preview hardening. This order prevents Moneta and embodiment work from being built on scientifically false or computationally unbounded evidence.

**Current interpretation:** P1-A, P1-B, P1-C, P1-D, P1-E and P1-F contain material implementation advances but remain **IMPLEMENTATION LANDED / REVIEW ACTIVE**, not `VERIFIED COMPLETE`. The dominant risks are scientifically invalid evidence, residual resource-control bypasses, memory/transport cliffs and product/device evidence gaps. Stream A remains free to advance work whose dependencies are stable; Stream B fixes correctness and evidence foundations ahead of further representation breadth.
...
| RF-028 | Scientific validity / temporal & spectral | **Blocker** | The merged baseline still permits temporal/spectral paths whose scientific meaning needs independent closure: FFT/seasonality/trend evidence must honor authoritative elapsed time, sampling regularity and physical units rather than retained observation rank. The skipped #427/#428 line is not merged evidence. | Land a clean, independently verified fix-forward for actual-time pairing, regularity checks, physical frequency units, irregular/gapped fail-closed or governed estimation, row-shuffle/time-rescale/gap metamorphic tests and method-specific provenance before treating this blocker as closed. |
| RF-029 | Scale / memory & resource envelope | **Blocker for 10M claim** | WASM is capped at 512 MiB while primitive resident storage uses f64 values plus validity and operations allocate additional point/transposition/output buffers. A generic “10M rows” claim is therefore false without dimensional/workload constraints; six numeric dimensions alone exceed the cap before runtime overhead. | **IMPLEMENTATION PARTIAL:** #429 landed the shared Rust budget vocabulary, saturating work estimates, row/PointCloud transient estimates, dense CSR accounting and canonical clustering preflight. Current RF-030 work extends Rust-owned estimates to production TDA and surfaces typed cross-ABI refusal. Remaining: explicit 10M workload/device profile, complete resident+transient peak accounting, kernel-inline/direct-caller enforcement, streaming/chunking/validity compaction where justified, durable refusal/approximation provenance and measured peak-memory evidence. |
| RF-030 | P1-C / high-D complexity | High | RF-018's soundness fix makes `GridSparseIndex` fall back to exact all-pairs search for d>6. Large high-dimensional Mapper/neighbourhood workloads can therefore cross a silent O(N²d) performance cliff while nominally requesting sparse mode. | **IMPLEMENTATION PARTIAL on `fix/rf030-tda-resource-envelope`:** the canonical production TDA bridge now consults a Rust-owned preflight before Mapper/persistence/Betti execution. The estimator uses complete-case validity, conservatively uses source-row count if feature borrowing is unavailable, includes Mapper `bins`, Betti `steps`, dense CSR and duplicate edge buffers, and refuses over-budget high-D fallback with typed `UnsupportedAtScaleError`; a real-WASM 9,000×7 test proves the production seam. Remaining: raw WASM `data_compute_*` exports and direct Rust/shared-neighbourhood callers can still bypass the sibling preflight; no governed bounded approximation is selected; refusal is not yet durable analytical provenance/NIL; conservative dense-output budgeting intentionally withholds some large d≤6 cases until output growth is bounded. |
...
| RF-032 | Evidence classification / topology inference | High | Rust topology inference on merged main still has loose name-driven semantics; the attempted tightened implementation in the skipped temporal/topology PR line is not merged evidence. | Land a clean fix-forward using exact/structured aliases plus type/range/corroboration checks. Keep `x`/`y` as exact coordinate names only and do not let them alone prove geospatial semantics. Add false-positive adversarial schema tests and converge all topology evidence authorities per RF-036. |
...
#395 closed production JS TDA rematerialisation; #405 enabled typed/columnar-only handles to execute persistence, Mapper and Betti-0 directly in Rust with `ingestMode` provenance and real-WASM boundary tests. #423 introduced the shared point-access substrate; #426 corrected missing-value semantics via complete-case eligibility and source-row mapping. The current RF-030 branch adds a Rust-owned production preflight and typed unsupported-at-scale bridge outcome but is not merged evidence yet.

This remains **IMPLEMENTATION LANDED / REVIEW ACTIVE** until policy/exclusion/resource-refusal metadata is durable in analytical provenance, kernel/direct-caller scale work is bounded by RF-029/RF-030, CI evidence is clean and independent review agrees.
...
#410-#413 established sparse-neighbourhood, perceptual-fitness, semantic-target/focus-context and actionable-NIL components. Stream B fixes through #416/#418/#419/#420/#421/#426 materially improved them. #429 adds merged operation resource guards; RF-030 is the current unmerged TDA/high-dimensional enforcement tranche. They remain **IMPLEMENTATION LANDED / REVIEW ACTIVE** until their residual scientific, scale and physical-product evidence exits are met.
...
| 1 — Dataset Evidence | **MIGRATION AUTHORITY COMPLETE / SCIENCE ACTIVE** | Finish RF-007 provenance semantics; close RF-028 temporal truth and RF-032/RF-036 topology classification authority plus measurement semantics. |
...
- [ ] **RF-029/RF-030 scale:** current branch guards the canonical production bridge with Rust-owned preflight, but kernel/raw direct callers and durable refusal provenance remain open;
...
- [ ] **RF-029:** define supported scale profiles and measured resident/transient budgets; current RF-030 estimator conservatively counts PointCloud/CSR/transposition/edge-output memory for TDA;
- [ ] **RF-030:** merge and verify the production TDA preflight, then close the raw WASM/direct-neighbourhood bypass or make the kernel boundary itself enforce the budget;
- [ ] add explicitly governed approximation only with provenance and exact-vs-approximation quality evidence;
...
- [ ] **RF-032/RF-036:** prevent topology-name heuristics or cross-layer classifier disagreement from manufacturing semantic structure;
...
2. Do not consume temporal/spectral periodicity as stronger scientific evidence until RF-028 closes on merged, reviewed code.
...
1. **RF-007 + RF-017 — validity-aware analytical substrate.** Correctness landed in #426; provenance/scale residuals remain under RF-007/RF-029.
2. **RF-028 + RF-032 + RF-036 — temporal/evidence integrity.** The skipped #427/#428 line is not merged evidence; land a clean later fix-forward for actual temporal geometry and one canonical topology evidence authority.
3. **RF-029 + RF-030 + RF-031 + RF-035 — analytical resource envelope. CURRENT TRANCHE.** #429 landed the shared clustering guard; current RF-030 protects the canonical production TDA path with Rust-owned preflight and typed refusal. Next close kernel/direct-caller bypass and durable refusal provenance or explicitly carry those as residuals, then remove RF-035 large mutation rematerialisation and finish with a measured 10M qualification profile.
