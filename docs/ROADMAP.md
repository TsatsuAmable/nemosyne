...
| RF-006 | P1-U / world semantics | High | TechnoCore lens methods are not yet demonstrated as wired investigator input/analysis controls; IceVault remains a persistent largely decorative glyph. | **Fixed in P1-U5**: TechnoCore wired through RecommendationPanel with four diagnostic views (Guidance, Alternatives, Constraints, Remediation); typed NIL remediation applied via `_applyRemediation` → `recordRemediation`; replay via `reconstructRequirementsAndReArbitrate` using `requirementPatch` from `RemediationProvenance`. IceVault archival work remains in P1-U6. |
...
| RF-046 | Investigation provenance / digest completeness | High | The SHA-256 investigation digest is cryptographically strong but its preimage is a lossy projection: command parameters/result identities, much analytical provenance, finding/observation/annotation semantics and parts of representation evidence are omitted or reduced to counts. Semantically different investigations can therefore commit to the same digest contract. | **IMPLEMENTATION LANDED / REVIEW ACTIVE:** v2 semantic digest (`sha256-canonical-investigation-v2`) commits per-entity canonical hashes (events, results, structures, observations, findings, annotations, representation state, discoveries, NIL outcomes, research context) composed into a deterministic root; normalized to exclude presentation-only state and replay-volatile metadata (root-level timestamps, sessionId, eventId) while preserving nested semantic `timestamp` parameters. Mutation/tamper tests verify every governed semantic field changes the digest and presentation-only fields do not. Legacy v1 compatibility via `legacyDigestSchemaV1` option. **CURRENT Stream-B foundation item after RF-048.** |
...
| RF-051 | P1-A/P1-B / JavaScript scale cliffs | High | Even with Rust owning analytical work, ordinary JS preprocessing still contains N-dependent cliffs: `Dataset.rangeOf()` materializes values and spreads them into `Math.min/Math.max`, append uses spread into `push`, and DatasetSpace clones/hashes rows and may compute ranges in JS. These can hit browser argument/memory limits before Rust's resource envelope helps. | **IMPLEMENTATION LANDED / REVIEW ACTIVE:** `Dataset.rangeOf` and `updateRows` use bounded loops (no spread); `DatasetSpace` avoids clone/full JSON when pre-computed values provided (`datumIds`, `ranges`, `fingerprint`); `AtlasCore` uses typed column payload (`_buildTypedPayloadFromDataset`) for worker registration on large datasets (≥50k rows), avoiding JSON round-trip; `DatasetIdentity` adds async streaming SHA-256 (`canonicalDatasetIdentityHexAsync`) using Web Crypto API for datasets ≥50k rows; capacity tests in `dataset-large-n-guards.test.ts` verify bounded loops, append without spread, and typed payload structure. Remaining: complete resident+transient peak accounting across Rust+Worker+JS, streaming/chunking/validity compaction where justified, measured browser/WASM capacity evidence. Integrate with RF-029/RF-035. |
...
- [x] close **RF-006** through P1-U5/P1-U6;
