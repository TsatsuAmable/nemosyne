...
**Current main at the start of the latest review pass:** `6db1127` (#418, merged; all CI green). #416 (Stream B P1-D/P1-F hardening — RF-022, RF-026) and #418 (P1-C sparse topology review exit — RF-017, RF-018, RF-019, RF-020, RF-021) have both landed. Earlier merged: #410 (P1-C sparse topology), #411 (P1-D perceptual fitness), #412 (P1-F semantic target/focus-context), #413 (P1-E actionable-NIL), #414 (P1-B async runtime production wiring), #415 (nemosyne-data corpus connector). Neighbourhood RF-017/RF-018/RF-021 fixes (farthest-point sampling, validity normalization, d>6 ExactIndex fallback) landed in the neighbourhood.rs commit preceding #418.
...
**In flight:** RF-025 tranche (P1-F production wiring) on branch `fix/review-stream-p1f-production-wiring-rf025` — `SemanticTargetResolver` + `FocusContextController` wired onto the real `InputRouter` picking path via new `InteractableRegistry.raycastSceneAll()`; live interactables populated with durable `semantic` metadata (data nodes = `observation`, structure handles = `cluster-region`/`persistence-structure`/`mapper-node`); structure selection drives `focusContext.focusStructure` + `onFocusChange`; durable focus snapshot persisted/restored through `PresentationState.focus` + `WorldSessionController`. Gates green locally (tsc · eslint 0 errors · 297 vitest · 204 cargo); PR pending. Just landed: #420 (RF-027 — typed hard-constraint codes, deviceFeasibility permissibility/feasibility split, durable remediation provenance), #419 (RF-023/RF-024).
