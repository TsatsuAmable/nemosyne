...
| UXR3 | **PARTIAL / ACTIVE NEXT FRONTIER** | #763 bounds semantic-materialisation cardinality, queueing, per-tick work and production refined-detail admission; #764 adds stable coarse-to-refined replacement. The current tranche adds symmetric collapse, identity-only eviction descriptors and reconstruction through the resident Worker-backed production detail path, with exact fingerprint/generation/version/decision/semantic identity checks. Generic family payload/cost bounds, broader streaming and physical qualification remain open. |
