...
**Current remote main:** `262207e` (`ci: shard Vitest coverage with merged global gate` — #438 merged; preceded by #437 Stream C security assurance + CI parallelization). The preceding Stream-B tranches landed: #426 (RF-007/RF-017 missing-data correction), #428 (RF-028/RF-032 temporal/evidence integrity), #429 (first RF-029/RF-030/RF-031/RF-035 analytical-resource-envelope cut: shared `AnalysisBudget`/`ResourceEstimate` vocabulary, canonical-operation-ABI preflight for K-means/hierarchical/DBSCAN, stable `UNSUPPORTED_AT_SCALE` refusals), #431 (RF-030 production TDA preflight: Rust-owned `data_tda_resource_preflight` gating Mapper/persistence/Betti with typed refusal), #432 (VR UI design-system docs), #433 (roadmap status refresh), #434 (CodeQL randomness remediation — clean zero-finding baseline for the current JavaScript/TypeScript query set), #435 (RF-030 kernel-inline enforcement: the Rust `data_compute_*` exports refuse over-budget work in-band, closing the direct/raw-caller bypass), #436 (roadmap status refresh), #437 (Stream C security assurance + CI parallelization), and #438 (Vitest coverage sharding with merged global gate). The durable-refusal-provenance tranche (`feat/rf030-durable-refusal-provenance`) lands the RF-030 residual that #435 left open. Static resource limits remain kernel safety guards, not Quest qualification and not evidence of generic 10M-row support.

**Just-landed Stream-B tranche:** #437 (Stream C security assurance + CI parallelization) merged green, and the durable-refusal-provenance tranche (`feat/rf030-durable-refusal-provenance`) lands the RF-030 residual that #435 left open. #435 closed the RF-030 direct-caller bypass: the Rust `data_compute_mapper_graph` / `data_compute_persistence_intervals` / `data_compute_betti0_curve` exports run the resource preflight themselves and, on refusal, write `{ unsupportedAtScale: true, preflight }` to the output buffer instead of a result, via the same `(out_ptr, out_len) -> u32` ABI; no caller — production bridge or direct/raw export — can bypass the analytical resource envelope; the TS `tdaCall` translates the in-band refusal to `UnsupportedAtScaleError`; the standalone `data_tda_resource_preflight` remains as a dry-run query. The durable-refusal-provenance tranche now makes that refusal **kernel-authoritative and durable**: the Rust refusal branches call `record_refusal` to write a `Provenance` envelope with `outcome: "refused"` and `outputFingerprint: ""` to the `LAST_PROVENANCE` side-channel (the in-band envelope stays size-stable with no timestamp, preserving the two-call `(probe → write)` ABI); the TS `tdaCall` reads `kernelProvenance()` and attaches it to the typed error; `AtlasCore.recordRefusalFromError` appends a non-mutating `kind: 'refusal'` `RefusalProvenance` event to the `EvidenceLedger` (persisted by `NemosyneSession` at schemaVersion 2, unchanged); the sync adapter intercepts `UnsupportedAtScaleError` before the failure funnel (a refusal is NOT a kernel failure — `onKernelFailure`/`markKernelUnavailable` are not invoked) and rethrows; the async worker surfaces `result.refusal`, the port reconstructs the typed error, records it, and rejects. A real-WASM regression test proves a raw direct export call still cannot bypass the envelope and that the side-channel carries the refusal provenance. **Review-active residuals:** governed bounded approximation with exact-vs-approximation quality evidence, RF-035 large-mutation rematerialisation, and measured workload/device evidence remain open under RF-029/RF-031.

**Reprioritised Stream-B critical path:** (1) RF-007/RF-017 validity correctness landed in #426, with provenance/scale residuals; (2) RF-028/RF-032 temporal/evidence integrity landed in #428, with RF-036 authority-convergence and irregular-series-spectral residuals open; (3) **CURRENT:** RF-029/RF-030/RF-031/RF-035 analytical resource envelope (#429/#431/#435 merged; kernel-inline/direct-caller enforcement landed; durable refusal provenance landed; residual governed approximation provenance, RF-035 large-mutation rematerialisation, and measured workload/device-evidence work open); (4) RF-001/RF-002 representation truth; (5) RF-005/RF-006/RF-008 plus the RF-027 World/UX carry-over; (6) RF-015/RF-033 production evidence; (7) physical Quest 3S qualification; (8) private-preview hardening. Stream C runs in parallel on RF-037 through RF-043. The dependency rule is: **valid data geometry → valid analytical evidence → bounded computation → faithful representation → coherent investigator UX → physical XR proof → private preview.**
...
- [ ] **RF-029/RF-030 scale:** #431 guards the canonical production TDA bridge with Rust-owned preflight, #435 makes that enforcement kernel-inline (direct/raw callers cannot bypass the budget), and the durable-refusal-provenance tranche makes the refusal kernel-authoritative and durable (side-channel `outcome: "refused"` provenance + `RefusalProvenance` ledger event); governed approximation provenance and measured scale qualification remain open;
...
- [x] **RF-030 durable refusal provenance:** the refusal is now kernel-authoritative and durable — the Rust refusal branches write `outcome: "refused"` provenance to the `LAST_PROVENANCE` side-channel (envelope stays size-stable for the two-call ABI), the TS bridge attaches it to `UnsupportedAtScaleError`, `AtlasCore.recordRefusalFromError` appends a non-mutating `kind: 'refusal'` `RefusalProvenance` event to the `EvidenceLedger` (persisted at schemaVersion 2), and the sync/async paths record-then-rethrow so `onKernelFailure`/`markKernelUnavailable` are NOT invoked;
...
3. **RF-029 + RF-030 + RF-031 + RF-035 — analytical resource envelope. CURRENT TRANCHE.** #429 landed the shared clustering guard, #431 landed canonical production TDA preflight/typed refusal, #435 closed the kernel/direct-caller bypass by making the `data_compute_*` exports enforce the budget in-band, and the durable-refusal-provenance tranche made the refusal kernel-authoritative and durable (side-channel `outcome: "refused"` provenance + `RefusalProvenance` ledger event, sync+async record-then-rethrow, session round-trip). Next remove RF-035 large mutation rematerialisation, govern any approximation explicitly with exact-vs-approximation quality evidence, and finish with measured workload/device qualification.
