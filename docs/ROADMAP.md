...
**Current remote main at the start of this Stream-B tranche:** `da7e54e` (#426, `fix(tda): honor missing-value validity in metric spaces`). #426 landed the RF-007/RF-017 complete-case metric/TDA correctness fix and source-row identity preservation. P1-A/P1-C remain review-active for provenance and scale rather than because missing values are still being manufactured as geometric zero.

**Current Stream-B tranche:** `fix/rf029-analytical-resource-envelope` is implementing RF-029/RF-030/RF-031/RF-035 in small falsifiable cuts. The first cut is **IMPLEMENTATION PARTIAL**: Rust now owns a shared `AnalysisBudget`/`ResourceEstimate` vocabulary with saturating work estimates and conservative transient-memory estimates; worst-case dense CSR payloads are counted rather than only offsets; and the canonical serialisable operation ABI preflights K-means, hierarchical clustering and DBSCAN before expensive matrix/neighbourhood work. Refusals use stable `UNSUPPORTED_AT_SCALE` metadata carrying operation, decision, reason, rows, dimensions, work and transient bytes. These defaults are kernel safety limits, not Quest qualification and not evidence of generic 10M-row support. RF-030 TDA/shared-neighbourhood enforcement, cross-ABI typed refusal/provenance, RF-035 large-mutation residency and an explicit 10M qualification profile remain open.

**Adversarial review note for the current cut:** the initial resource estimate undercounted radius-neighbourhood memory by omitting possible CSR edge payloads. That defect was corrected before PR creation by budgeting the worst-case `n(n-1)` directed edge storage. A second review found that the current naïve k-means++ implementation recomputes distances to every existing centroid on each seed round, making seeding O(N·D·K²) and the user-controlled worst case cubic when K grows with N; the budget now models that implementation rather than the textbook optimized variant. Production clustering is guarded at the canonical operation bridge, but raw internal Rust helpers and TDA topology helpers remain potential bypasses; RF-031 and RF-030 therefore must not be marked verified complete.

**Reprioritised Stream-B critical path:** (1) RF-007/RF-017 validity correctness landed in #426, with provenance/scale residuals; (2) RF-028/RF-032 temporal/evidence work remains unmerged and review-active; (3) **CURRENT:** RF-029/RF-030/RF-031/RF-035 analytical resource envelope; (4) RF-001/RF-002 representation truth; (5) RF-005/RF-006/RF-008 plus the RF-027 World/UX carry-over; (6) RF-015/RF-033 production evidence; (7) physical Quest 3S qualification; (8) private-preview hardening. This order prevents Moneta and embodiment work from being built on scientifically false or computationally unbounded evidence.
