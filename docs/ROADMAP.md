...
**Current remote main:** `01b987f` (`fix(wasm/tda): RF-030 enforce resource envelope kernel-inline` — #435 merged). The preceding Stream-B tranches landed: #426 (RF-007/RF-017 missing-data correction), #429 (first RF-029/RF-030/RF-031/RF-035 analytical-resource-envelope cut: shared `AnalysisBudget`/`ResourceEstimate` vocabulary, canonical-operation-ABI preflight for K-means/hierarchical/DBSCAN, stable `UNSUPPORTED_AT_SCALE` refusals), #431 (RF-030 production TDA preflight: Rust-owned `data_tda_resource_preflight` gating Mapper/persistence/Betti with typed refusal), #432 (VR UI design-system docs), and #435 (RF-030 kernel-inline enforcement: the Rust `data_compute_*` exports refuse over-budget work in-band, closing the direct/raw-caller bypass). Static resource limits remain kernel safety guards, not Quest qualification and not evidence of generic 10M-row support.
