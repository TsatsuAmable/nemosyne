...
**Adversarial review note for the current cut:** the initial resource estimate undercounted radius-neighbourhood memory by omitting possible CSR edge payloads. That defect was corrected before PR creation by budgeting the worst-case `n(n-1)` directed edge storage. K-means complexity metadata is classified conservatively as quadratic because user-controlled `k` may grow with `n`. Production clustering is guarded at the canonical operation bridge, but raw internal Rust helpers and TDA topology helpers remain potential bypasses; RF-031 and RF-030 therefore must not be marked verified complete.
...
| RF-031 | Operations / computational budget | High | User-callable hierarchical clustering repeatedly compares cluster pairs and member pairs, with roughly cubic worst-case behavior; K-means cost grows with user-selected `k`; DBSCAN can allocate a dense radius CSR. Worker execution protects frame responsiveness, not process memory/time. | **IMPLEMENTATION PARTIAL on current branch:** the canonical serialisable operation bridge now preflights K-means, hierarchical clustering and DBSCAN in Rust before expensive work and emits deterministic `UNSUPPORTED_AT_SCALE` metadata on refusal. Tests cover pathological hierarchical work, dense DBSCAN CSR risk, large K-means work and preserved small workloads. Remaining: cross-ABI typed refusal/provenance, governed bounded alternatives where useful, direct/internal helper bypass review and authoritative CI/adversarial evidence. |
