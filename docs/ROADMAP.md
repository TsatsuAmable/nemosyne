...
**Current integration base:** `main@ac347fd1d8b4a52fb6efabc499c4296e4812b802` (#769). UXR2 bounded presentation-resource lifecycle is landed. UXR3 now has bounded semantic-materialisation cardinality/backpressure (#763), stable coarse-to-refined replacement (#764), and exact-identity collapse/evict/reconstruct continuity (#769) through the production Worker-backed detail path without retaining analytical payloads. The remaining UXR3 software frontier is generic semantic-family payload/cost governance plus bounded streaming across the families/paths that can still accumulate work. Worker/WASM release qualification, UXR4 evidence refocus, and physical UXR5 claims remain separate downstream gates.
...
- #769 closes the bounded UXR3 lifecycle-continuity tranche: symmetric coarse/refined replacement, identity-only eviction descriptors, fail-closed reconstruction matching and production-path reconstruction are exact-head verified. It does not close generic family payload/cost governance, broader bounded streaming, Worker/WASM release qualification or physical-device evidence.
...
5. **Then UXR4.** Refocus verification around interaction, responsiveness, frame/render, memory/resource and semantic-scale envelopes using the existing governed evidence chain. UXR5 physical Quest qualification follows when attributable device/human evidence is available. PT9 remains downstream until the declared P1-UXR software/evidence prerequisites are satisfied.
...
| UXR3 | **PARTIAL / ACTIVE NEXT FRONTIER** | #763 bounds semantic-materialisation cardinality, queueing and per-tick work; #764 establishes stable coarse-to-refined replacement; #769 adds symmetric collapse, identity-only eviction descriptors and fail-closed reconstruction through the resident Worker-backed detail path. **Next:** UXR3-F1 inventory and contract generic semantic-family payload/cost bounds; UXR3-F2 enforce those bounds on the first production family/path that lacks them; UXR3-S1 bound remaining streaming/chunk/transfer queues with cancellation and stale-generation refusal; UXR3-E1 cross-family scale/perturbation qualification and STOP review. Worker/WASM release and physical evidence remain claim-specific downstream gates. |
