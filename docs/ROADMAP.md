...
**Current forward TEC order:** finish the remaining TEC1 authority/identity/replay gaps first where they are prerequisite to downstream evidence references; then complete the bounded TEC2 migration debt (`ClusterProfile.density_variation`, `dependence.maxCorrelation`, and any inventory-confirmed residuals) under explicit contracts; then execute TEC3 production-path falsifier coverage from the audited gaps. Do not bundle these into one PR, and do not claim TEC closure from renamed fields or fixture count alone.
