...
**R1 transfer progress:** Mapper, persistence intervals, Betti-0, statistics and dataset JSON now use Rust-owned prepare/read/destroy result transfer through the production bridge. Deterministic tests cover one substantive computation/materialisation, legacy output parity, statistics provenance, JSON provenance neutrality, graph/row identity, admission, malformed input, host allocation/read failures, dataset destruction and generation reset. Invalid and columnar-only statistics/JSON requests preserve their null outcome even under saturation. This is partial R1 progress: semantic builders/detail and spectral result transfer remain before R1 closure and E1 requalification. No whole-load copy elimination, measured latency or physical-memory improvement is claimed.
