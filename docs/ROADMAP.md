...
**R1 bounded TDA tranche:** Mapper, persistence intervals and Betti-0 now use Rust-owned prepare/read/destroy result transfer through the production bridge. Deterministic tests cover one analytical computation, output/provenance parity, admission, malformed input, host allocation/read failures, dataset destruction and generation reset. This is partial R1 progress: statistics, dataset JSON, semantic builders/detail and spectral result transfer remain before R1 closure and E1 requalification. No measured latency or physical-memory improvement is claimed.
