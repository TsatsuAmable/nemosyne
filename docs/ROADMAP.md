...
**Current main at the start of the latest review pass:** `848f8a6` (#421, merged; all CI green). The three queued Stream B tranches have all landed: #419 (RF-023/RF-024 — P1-D evidence identity + study governance), #420 (RF-027 — P1-E actionable-NIL typed codes + durable provenance), #421 (RF-025 — P1-F semantic targeting + focus/context production wiring). Earlier: #416 (RF-022, RF-026), #418 (P1-C sparse topology review exit — RF-017/RF-018/RF-019/RF-020/RF-021), #410–#415.

**Next Stream B tranches (in roadmap order):** the queued P1-D/P1-E/P1-F chain is closed. Remaining open Stream B RFs: RF-007 (shared validity-aware point-access substrate), RF-001/RF-002 (P1-R) and RF-005/RF-006/RF-008 (P1-U), plus the RF-025 carry-over (wire the World/UX call site to `recordRemediation` when an investigator applies a remediation — paired with the P1-F path now in place) and the physical Quest 3S qualification blocker.

**In flight:** none. Last landed: #421 (RF-025 — `SemanticTargetResolver` + `FocusContextController` wired onto the real `InputRouter` picking path via `InteractableRegistry.raycastSceneAll()`; live interactables carry durable `semantic` metadata; structure selection drives `focusContext.focusStructure` + `onFocusChange`; durable focus snapshot persisted/restored through `PresentationState.focus` + `WorldSessionController`). #420 (RF-027), #419 (RF-023/RF-024) before that.

**Current interpretation:** P1-C, P1-D, P1-E and P1-F are useful implementation advances, but independent review found correctness, scale, scientific-semantics, production-wiring and evidence gaps. They are therefore **IMPLEMENTATION LANDED / REVIEW ACTIVE**, not `VERIFIED COMPLETE`. The P1-F production-wiring gap (RF-025) is now closed; RF-026 remains review-active until the physical XR semantic-target/focus-context task evidence is captured. Stream A remains free to advance private-preview/scientific work where dependencies are stable; Stream B continues closing earlier and newly discovered RF findings.
