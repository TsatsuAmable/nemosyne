...
**R1 transfer progress:** Mapper, persistence intervals, Betti-0, statistics, dataset JSON, the five semantic embodiment builders and semantic detail now use Rust-owned prepare/read/destroy result transfer through the production bridge. Deterministic tests cover one substantive computation/materialisation, legacy output parity, statistics provenance, JSON provenance neutrality, graph/row identity, semantic decision provenance and detail membership, admission, malformed input, host allocation/read/parse failures, dataset destruction and generation reset. Invalid and columnar-only statistics/JSON requests preserve their null outcome even under saturation; stale semantic-detail requests retain their structured refusal rather than becoming null. This is partial R1 progress: spectral result transfer and the remaining production-export inventory precede R1 closure and E1 requalification. No whole-load copy elimination, total semantic-memory bound, measured latency or physical-memory improvement is claimed.
...
6. **Then UXR4.** Refocus verification around interaction, responsiveness, frame/render, memory/resource and semantic-scale envelopes using the existing governed evidence chain.
7. **Then UXR5.** Complete attributable physical Quest qualification and the finite P1-UXR STOP review. Quest is a specialist modality and reference stress environment; its completion does not make VR hardware a prerequisite for later investigator participation.
...
10. **Then PT9.** Begin the Moneta learning-evidence pipeline only after WP deployment and WQ qualification establish a viable route to sufficiently broad, governed investigator judgments.
...
| UXR3 | **PARTIAL / S1 STOP, R1 ACTIVE NEXT, E1 DOWNSTREAM** | #763 bounds semantic-materialisation cardinality, queueing and per-tick work; #764 establishes stable coarse-to-refined replacement; #769 adds symmetric collapse, identity-only eviction descriptors and fail-closed reconstruction through the resident Worker-backed detail path; #779 bounds Worker execution/registration admission; #781 adds safe all-stale Worker recycling; #783 adds bounded lifecycle outcomes and stale/mismatched-result disposition. `review/UXR3_S1_CLOSURE_REVIEW_2026-09-19.md` records the finite S1 PASS/STOP. The 20 September runtime review then established duplicate authoritative computation in the generic two-call Rust/WASM result ABI. **Next:** UXR3-R1 single-computation result transfer, then UXR3-E1 cross-family scale/perturbation qualification and STOP review. Worker/WASM software lifecycle closure does not establish physical memory release timing, latency, Quest fitness, or human evidence. |
