...
**Current main at the start of the latest review pass:** `4ab6d9a` (#422, merged; all CI green). The three queued Stream B tranches have all landed: #419 (RF-023/RF-024), #420 (RF-027), #421 (RF-025). Earlier: #416 (RF-022, RF-026), #418 (P1-C sparse topology review exit — RF-017/RF-018/RF-019/RF-020/RF-021), #410–#415.

**Next Stream B tranches (in roadmap order):** RF-007 (shared validity-aware point-access substrate) is in flight — the scale/shared-substrate half is fixed; the **validity-honoring half is a governed design decision pending user adjudication** (skip vs mask; changes TDA outputs + the row-path `unwrap_or(0.0)` parity contract). Then RF-001/RF-002 (P1-R) and RF-005/RF-006/RF-008 (P1-U), plus the RF-025 carry-over (wire World/UX to `recordRemediation`) and the physical Quest 3S qualification blocker.

**In flight:** RF-007 tranche (P1-A/P1-C shared point-access substrate) on branch `fix/review-stream-rf007-shared-point-access` — new `data::point_access` substrate with the documented columnar primitive invariant; `FeatureSpace::from_columnar` borrows via the substrate (no throwaway per-column clone, byte-identical); `PointCloud::from_columnar` delegates lookup+semantics to the same substrate; 7 new Rust regression tests (invariant, cross-substrate parity, from_parts non-finite, borrowed byte-identity). Gates green locally (tsc · eslint 0 errors · 211 cargo · vitest). PR pending. Last landed: #421 (RF-025), #420 (RF-027), #419 (RF-023/RF-024).
