...
- **2026-08-20 — Moneta Dataset-Centric Analytical Representation Architecture Migration (✅, on `feat/moneta-analytical-representation-architecture`):**
  - **Evolved Draco → Moneta:** Transitioned the analytical representation solver from observation-centric grammar of graphics to dataset- and task-centric analytical representation theory (`Moneta`). Preserved Rust as the sole analytical authority, Atlas as the semantic intent/meaning spine, and Spatial Runtime as embodiment.
  - **Rust/WASM DatasetStructureProfile (`wasm/src/data/profile.rs`):** Exported `data_compute_structure_profile` and capability flag `CAP_MONETA_RUST` (1 << 3) providing rich dimensionality, distribution, correlation, cluster, density, temporal, spatial, graph, hierarchy, anomaly, missingness, categorical, and spectral profiles.
  - **2-Stage Hypothesis & Utility Solver (`MonetaHypothesisEngine`):** Implemented two-stage constraint reasoning: Stage 1 Hard Constraint Filtering (topological validity, scale limits, dimensionality preconditions) + Stage 2 Information-Preservation Utility Scoring (task alignment, information preservation goals vs loss budget, empirical priors). Emits `RepresentationDecision` with full machine-readable provenance and explanation traces.
  - **12 Semantic Representation Candidates & 8 Spatial Embodiment Layouts:** Built semantic representations (`SCATTER_CLOUD`, `CORRELATION_MATRIX`, `TIME_TRAJECTORY`, `RELATIONSHIP_GRAPH`, `HIERARCHICAL_SPACE`, `GEOGRAPHIC_TERRAIN`, `FLOW_STREAMLINES`, `SPECTRAL_LANDSCAPE`, `CLUSTER_REGIONS`, `DISTRIBUTION_DENSITY`, `ANOMALY_LANDSCAPE`, `PARALLEL_COORDINATES_3D`) and decoupled them from spatial embodiment layouts (`GridLayout3D`, `ForceDirected3D`, `RadialTreeLayout`, `TimeSeriesRibbonLayout`, `StreamlineLayout`, `GeoSurfaceLayout`, `SpectralVolumeLayout`).
  - **Evidence-Informed Recommender Loop (`EvidenceStore` & `EvidenceWeightedScorer`):** Connected empirical crossover trial results (`StudySessionExport`) directly to candidate scoring adjustments.
  - **Full Backward Compatibility Shims:** Added comprehensive shims in `src/draco/` preserving all existing `Draco*` class, type, and module interfaces across the codebase.
  - **Gates:** `tsc --noEmit` 0 errors · `eslint` 0 errors (167 warnings, pre-existing test files) · `cargo test` 89/89 passed · `npm test` 247/247 test files passed (1,608 passed / 26 skipped jsdom-WASM parity by design) · `npm run test:smoke` passed · `npm run build` exit 0 (162ms).
