...
- **2026-08-20 — Functional-Fitness Remediation & Comprehensive Architecture Hardening (✅, on `feat/moneta-analytical-representation-architecture`):**
  - **Workstream 1 (Production WASM & Fail-Closed Boot):** Unified build scripts (`package.json`, `vite-wasm-pack-plugin.js`, `dev/wasm-serve-server.ts`), wired production WASM delivery (`/wasm/pkg/nemosyne_wasm_bg.wasm`), introduced explicit `bootState` in `World.ts` and fail-closed bootstrap messaging.
  - **Workstream 2 (Atlas Lifecycle, Handles, Versions, Error Boundaries):** Added atomic `commitKernelResult` on `AnalyticalState.ts`, deterministic dataset versioning, exception-safe kernel handle cleanup, cycle-detecting DFS validation in `InvestigationGraph.ts`, and state roundtrip in `InvestigationAggregate.ts`.
  - **Workstream 3 (Trustworthy Rust Profile & Single Moneta Authority):** Replaced synthetic profile constants in `wasm/src/data/profile.rs` with genuine algorithms (IQR outlier bounds, 8-bin peak/valley multimodality, k-means silhouette, BFS connected components, DFS cycle detection, tree depth/branching factor, z-score anomaly scoring).
  - **Workstream 4 (Canonical Investigation, DAG, Persistence, Digest, Package, Replay):** Eliminated insecure hash fallbacks in `InvestigationDigest.ts` (authentic SHA-256 with fail-closed `CapabilityError`), added zip-slip/bomb bounds to `NemosynePackage.ts` (`MAX_ARCHIVE_SIZE = 100MB`, `MAX_TOTAL_UNCOMPRESSED = 250MB`), and sanitized `ShareableSessionURL.ts`.
  - **Workstream 5 (Live Ingestion & Stream Processing):** Fixed replace/window buffer semantics in `LiveStreamCoordinator.ts`.
  - **Workstream 6 (World-Space Pose Correctness & VR Lifecycle):** Fixed `MarkMomentAction.ts` and `DiegeticErrorBoundary.ts` world-space coordinates via `getWorldPosition()` and `getWorldQuaternion()`.
  - **Workstream 7 (Collaboration Correctness & Lifecycle):** Updated `CollaborationCoordinator.ts` to transmit true world headset pose and ensure comprehensive `PeerAvatarManager.ts` disposal on room exit.
  - **Workstream 8 (Single Gesture Recognition Authority):** Arbitrated single active gesture recognizer per frame in `WorldInputCoordinator.ts`.
  - **Workstream 9 (Study Instrument Governance & Crossover Analysis):** Labeled `FrozenStudyConfig.ts` with draft status, implemented strict phase transition validation and interaction logging in `ExperimentRunner.ts`, upgraded `StudyStatisticalAnalyzer.ts` to paired crossover difference $t$-tests with Cohen's $d_z$, and added RFC 4180 CSV escaping with spreadsheet formula injection protection in `StudyDataExporter.ts`.
  - **Workstream 10 (Performance Budgets, Clean Gates, & Code Quality):** Fixed `PerformanceBudget.ts` drop window pruning, achieved 0 `tsc` typecheck errors, 0 ESLint errors, 89/89 Rust tests passing, 247/248 Vitest test files passing (1,608 passed / 26 skipped jsdom-WASM parity by design), and clean production build.
  - **Gates:** `tsc --noEmit` 0 errors · `eslint` 0 errors (167 warnings, test-scoped) · `cargo test` 89/89 passed · `npm test` 247/247 test files passed (1,608 passed / 26 skipped jsdom-WASM parity by design) · `npm run build` exit 0 (179ms).

  - **P0.1 Remove Insecure Cryptographic Fallback:** Rewrote `SignedTicketVerifier` to use a single HMAC-SHA256 implementation via Web Crypto SubtleCrypto — no FNV-1a or ad-hoc fallback. Added `CryptoCapabilityError` typed error for fail-closed behaviour when Web Crypto is unavailable. Added `TicketErrorKind` discriminated union (8 error kinds) and `timingSafeEqualBytes` constant-time byte comparison. Added 10 adversarial tests covering malformed signatures, tampered payloads, expired/future tickets, replay, and capability-unavailable scenarios.
  - **P0.2 Make Collaboration Security Fail-Closed:** Introduced explicit `SecurityProfile` (`Development | ResearchPreview | Production`) for `createRoomRegistry()`. Changed `allowOpenNoToken` default from `true` to `false` (fail-closed). Added `getSecurityDiagnostic()` returning typed warnings. Production requires authToken + allowedOrigins. Development restores frictionless open mode. Added 6 fail-closed profile tests.
  - **P0.3 Eliminate URL-Token Authentication:** Added `acceptUrlToken` option — defaults to `false` for Production, `true` for Development/ResearchPreview. In Production, URL-supplied tokens are ignored; peers must authenticate via in-band `auth` messages. Updated standalone `SignallingServer.mjs` to skip URL token extraction in Production. Added 3 P0.3 tests.
  - **P0.4 Real InteractionModeController State Machine:** Replaced permissive `validateTransition` with an explicit `TRANSITION_TABLE` adjacency graph (OBSERVE → TRANSFORM is illegal) and context-sensitive `TRANSITION_GUARDS` (TRANSFORM requires `hasSelection !== false`). Removed `default` fallthrough from `resolveBothPinchAction` — all 4 modes have explicit returns. Added 4 negative tests for illegal transitions, guard rejection, exhaustiveness, and invalid modes.
  - **P0.5 Audit & Remove Duplicate Semantic Authority:** Conducted a full state-ownership audit. Architecture is largely clean — the production path correctly delegates to the Rust kernel and Investigation is the sole persistent state authority. Fixed P20 (No Silent Fallbacks) violations: added `warnKernelLayoutUnavailable()` to all 6 layout files (GridLayout3D, ForceDirected3D, RadialTreeLayout, TimeSeriesRibbonLayout, GeoSurfaceLayout, StreamlineLayout) so WASM-unavailable fallbacks emit explicit degraded-state warnings instead of silently substituting JS computation. Added one-time warning to `normalize.ts inferType` (TS schema inference fallback for live streams). Remaining TS analytical fallbacks (Dataset.rangeOf/cardinalityOf, getDefaultEncodings, TS Draco solver) are explicitly acknowledged as staged migration paths, not silent duplicates.
  - **PeerPresenceHUD Render Suppression:** Added `_userEnabled` flag separating user preference from peer-driven visibility. `update()` suppresses rendering (`mesh.visible=false`) when no peers connected, avoiding permanent empty-state panel and saving per-frame canvas work.
  - **Gates:** `tsc --noEmit` 0 errors · `eslint` 0 errors (167 warnings, pre-existing) · `npm test` 239/239 test files passed (1,570 passed / 26 skipped jsdom-WASM parity by design) · `cargo test` 85/85 passed · `npm run build` exit 0 (147ms).

- **2026-08-20 — VR Spatial Clutter Audit & Functional Artifact Enforcement (✅, in progress on `feat/senior-platform-reproducibility-hardening`, uncommitted):**
  - **Lazy Panel Construction:** Converted `OperationLogPanel`, `InteractionCoach`, `NarrativeStrip`, and `LoadTestPanel` from unconditional boot construction to lazy `getOrCreate*()` accessors in `WorldUIManager`, eliminating boot-time GPU/draw-call cost for panels hidden in normal analyst sessions. The HandWheel "Log/Coach/Timeline" items now construct-on-demand.
  - **Superuser / Dev Lab Wheel Category:** Added a new `Dev Lab` wheel category surfacing the dormant UI components (`RepresentationCarousel`, `TransientContextCardManager`, `ProgressiveDisclosureController`, `SchemaMappingPanel`, `DracoDiagnosticHUD`, `GestureConfidenceHUD`, `FrustrationResponseManager`, `JITGestureHintManager`) for later review. Per the user directive, **no proposed panels/files were deleted** — all are preserved and reachable from the Dev Lab menu.
  - **PanelRolesManager `superuser` Role:** Added a new `superuser` panel role gated to DEVELOPER mode only, so Dev Lab panels cannot open outside developer mode.
  - **DracoDiagnosticHUD Ownership Cleanup:** Removed a duplicate WorldUIManager-side construction path for `DracoDiagnosticHUD` (World already owns and rebuilds it per palace). The Dev Lab "Draco Diag" item now toggles World's existing `this.diagnostic` via `world._toggleDracoDiagnostic()`.
  - **Callbacks Wired:** Added `frustrationAnalyzer` and `getDataset` optional callbacks to `WorldUIManagerCallbacks`, passed from `World.ts`, so `FrustrationResponseManager` and `SchemaMappingPanel` can construct with real runtime state.
  - **Deferred (not done):** Blender `.glb` asset integration sprint (separate feature sprint). `SpatialAssetRegistry` cosmetic factories (`createTechnoCoreMonolith`/`createFarcasterGate`/`createIceVaultGlyph`) left in place — they are environment-art methods (not panels), only referenced by tests; preserved per the no-deletion directive.
  - **Gates:** `tsc --noEmit` 0 errors · `eslint` 0 errors (167 warnings, pre-existing) · `npm test` 239/239 test files passed (1,545 passed / 26 skipped jsdom-WASM parity by design) · `npm run build` exit 0 (151ms) · `npm run audit:hygiene` 8/8 dimensions passed.
