...
9. **Artifact identity and region-local identity are deliberately separated.** The envelope's canonical `datasetFingerprint` binds the artifact and provenance to the exact canonical dataset instance. That fingerprint is row-order-sensitive by design, so it must not be embedded in a region-local ID whose contract also requires row-order invariance. A V1 region `semanticId` is therefore derived deterministically from semantic-embodiment schema version, `CLUSTER_REGIONS`, partition-field identity, and the preserved canonical source partition label. Region IDs are unique within the fingerprint-scoped envelope, survive row permutation, and are unaffected by insertion of an unrelated lexically earlier group. Empty/missing canonical partition labels are unassigned and do not become a region. Non-empty labels are preserved as source identity rather than trimmed/rewritten into another label.
...
- Exact artifact identity is the canonical dataset fingerprint plus candidate/method/provenance envelope. Region-local semantic identity is stable within that fingerprint-scoped artifact and intentionally does not hash the row-order-sensitive dataset fingerprint.
...
- The C2 identity clarification does not change ranking or scientific meaning. It removes an implementation contradiction between row-order-sensitive canonical artifact fingerprints and row-order-invariant region-local IDs.
...
- envelope dataset fingerprint changes when canonical dataset order changes, while region-local semantic IDs and region payload summaries remain row-order invariant and unaffected by unrelated group insertion;
