...
P1-MCR does not own the statistical evidence architecture, but it depends on its closure. `docs/ROADMAP.md` now defines P1-TEC as the finite gate between P1-WQ and PT9. Before MCR1 or any externally evolved `RepresentationGenome` is promoted into production, the relevant P1-TEC evidence handoff must establish that:
...
MCR0 authority/schema bounds are now landed by #800. When the live roadmap next promotes P1-MCR after the P1-TEC/PT9/PT10 prerequisites, start with **MCR1 semantic binding and composed graph validation**.
