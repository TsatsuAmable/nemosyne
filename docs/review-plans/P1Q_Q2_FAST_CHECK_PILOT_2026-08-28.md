...
They are installed with `--no-save --package-lock=false --ignore-scripts` after the repository's normal `npm ci`. The workflow then proves `package.json` and `package-lock.json` remain unchanged.
...
Hosted execution records the property runner's wall-clock and maximum RSS using `/usr/bin/time`. Repository install time is not attributed to the core property engine, but any future required adoption must account for permanent install/cache impact as well as test execution time.

## Adoption criteria

Classify the TypeScript Q2 pilot only after hosted evidence exists:
