# RF-061 — World composition-root convergence
...
`src/vr/World.ts` remains the correct place to see and assemble the complete Nemosyne runtime, but it has accumulated too many additional roles: composition root, public compatibility facade, mutable-state hub, workflow orchestrator, presentation mediator and compatibility shim. The 24 August systematic audit already warned that earlier god-object refactors risked proving class existence rather than cohesion or dependency direction. Current main still exhibits that failure mode: several extracted coordinators accept World-shaped hosts/callback surfaces or reach back into broad UI/runtime state, while `World.ts` itself remains the integration point for dataset loading, representation lifecycle, session restore, WASM/runtime initialization, cross-cutting operation reactions, live/collaboration wiring and substantial presentation mutation.
...
- Application modules must not import or receive `World`.
- Presentation modules must not import or receive `World`.
- Input modules must not import or receive `World`.
- Session modules must not import or receive `World`.
- Live/collaboration/landmark feature modules must not import or receive `World`.
- Contracts live beside the consumer that needs them; do not replace World coupling with a giant shared `coordinators/types.ts` contract hub.
...
### RF-061A — architecture contract and fitness functions
...
- extend the promoted P1-Q/Q0 architecture policy with RF-061 dependency rules;
...
**Exit gate:** CI can reject a newly introduced World back-reference or broad host bag while accepting a narrow capability interface; no runtime behavior changes are required.

### RF-061B — typed semantic intent boundary

**Intent:** give all input modalities and UI surfaces one application-facing vocabulary before moving implementations.

Introduce a bounded `WorldIntent`/application-intent vocabulary and small routing/handler boundary, for example:
...
- the initial handlers may delegate to existing World methods to preserve behavior while callers migrate;
...
- do not turn the EventBus into universal control flow or route high-frequency hover/frame/pose updates through the command layer.

**Exit gate:** at least one representative dataset action, analysis action, history action and workspace/UI action are invoked by UI and input through the same semantic intent path with no duplicate semantic dispatch.

### RF-061C — dataset/representation workflow boundary
...
Owns resources that are born/replaced/disposed together:
...
It receives narrow scene/input/interaction/tooltip/fact capabilities, never `World`.

Dashboard, TDA, theme, status and autosave effects remain downstream projections/bindings rather than being absorbed into either class.

**Exit gate:** production dataset load traverses `LoadDatasetUseCase` and `RepresentationSurface`; `World` no longer performs the representation construction/swap/disposal workflow itself; Atlas/Rust/Moneta remain the same authorities; load/reload/replacement/dispose behavior is covered through the production path.

### RF-061D — projection/binding decomposition

**Intent:** replace `_subscribeDataOperationEvents()` orchestration gravity with small reaction functions, not another manager.

Use functions that subscribe, perform one coherent reaction and return a disposer, for example:

- `bindOperationPresentation(...)`;
- `bindTelemetryProjection(...)`;
- `bindAutosaveProjection(...)`;
- `bindNarrativeProjection(...)`;
- `bindRecommendationProjection(...)`;
- `bindLoadTestReporting(...)`.

Do not create `OperationEventCoordinator`, `PresentationCoordinator` or a generic event manager that collects the same dependencies again.

**Exit gate:** cross-cutting operation reactions can be installed/disposed independently; production behavior and ordering that is semantically significant remain covered; World no longer contains the broad event-reaction switchboard.

### RF-061E — session/presentation boundary
...
**Exit gate:** session code no longer receives a World-shaped host; restore does not patch World/Atlas/UI private state directly; restore/replay semantic digest and identity remain invariant while presentation state can differ where governed.

### RF-061F — live, collaboration and landmark ports
...
- collaboration receives presence/camera/interaction/UI capabilities needed for the feature and emits collaboration state/events; it must not rebuild the wheel menu or reach into unrelated World/UI orchestration;
- TechnoCore, Vault, Farcasters and other landmarks emit ordinary semantic intents or use narrowly scoped bind functions rather than broad `WorldLandmarkHost` access;
- preserve Stream C security authority: collaboration refactoring must not move authentication/replay/role enforcement into presentation/application convenience code.

**Exit gate:** live/collaboration/landmark production modules contain no World back-reference or broad World-shaped host; their service/security authority remains unchanged; P1-W can wire deployed endpoints without cementing a god dependency.

### RF-061G — analytical runtime ownership

**Intent:** move `_initWasmRuntime` orchestration only after the intent and representation seams are stable.

- introduce a narrow analytical-runtime owner/capability that handles worker-port setup, runtime import/init, capability state, Atlas kernel binding/rebinding and generation-aware teardown;
- keep `WorldLifecycleOwner` as the lifecycle state-machine owner rather than replacing it;
...
**Exit gate:** World starts/stops the analytical runtime through a narrow capability; generation/recovery tests remain green across repeated construct/start/dispose cycles and real Worker/WASM evidence remains owned by RF-015/RF-033.

### RF-061H — dev/load-test feature isolation

**Intent:** stop optional diagnostic infrastructure from inflating the core composition graph.

- move load-test/Quest instrumentation, diagnostic-only listeners and optional research tooling behind explicit dev/research feature installers;
- prove production bundles cannot accidentally depend on dev-only installers;
- preserve dataset-safe evidence semantics from P1-Q/Q3.

**Exit gate:** core World composition does not directly own optional diagnostic workflows; production reachability checks prove dev-only dependencies are absent from the shipped path.

### RF-061I — compatibility/test cleanup

**Intent:** retire migration scaffolding only after production callers have moved.

- migrate full-system World tests from private-field mutation toward public intents/API plus observable behavior;
- inject runtime/kernel factories explicitly where tests need deterministic failure/recovery rather than patching `_wasmRuntime`/`_wasmUnavailable`;
- test individual use cases/resource owners with narrow fakes;
- retire World compatibility getters/proxies and legacy callback aliases only after production consumers and tests no longer require them;
- collapse or relocate broad shared coordinator types once consumers own narrow local contracts.

**Exit gate:** World private-state mutation is no longer required for ordinary application behavior tests; compatibility aliases are removed only with production-call-graph evidence; class existence alone is never treated as architectural completion.

## Sequencing and interaction with the existing roadmap

1. **Do not displace the current RF-029/RF-035/RF-051 measured scale/resource blocker work.** RF-061 is high-value architecture convergence, not a reason to pause the current measured whole-pipeline resource tranche.
2. **RF-061A is immediate and parallel-safe** because it is a design/policy tranche. It should extend the already-promoted P1-Q/Q0 architecture boundary machinery rather than introduce a second policy engine.
3. **RF-061B is also parallel-safe** once A lands because callers may move to intents while handlers still delegate to existing World behavior.
4. **RF-061C/D/F coordinate with P1-U file ownership.** Land them in bounded slices that avoid large merge-conflict surfaces while P1-U remains active; do not mix UI redesign with dependency refactoring in the same PR unless the seam is inseparable.
5. **RF-061E should land before durable private-preview session/replay claims are frozen.** It reuses RF-046/RF-047/USIM-A presentation-independence evidence rather than inventing another replay path.
6. **RF-061F must land before P1-W service wiring is considered stable**, because P1-W should wire deployed collaboration/live services through narrow ports, not preserve World backreferences.
7. **RF-061G follows C/E/F** so runtime extraction is driven by a clear remaining ownership seam rather than by line-count pressure.
8. **RF-061H/I are cleanup tranches**, not prerequisites for early architectural benefit, but must finish before RF-061 can be `VERIFIED COMPLETE`.

## Required architecture fitness functions

The programme should leave durable automated checks for:
...
- each mutable state domain has one declared owner;
- every resource owner has idempotent disposal and repeated start/dispose coverage where applicable;
- session restore uses application use cases/intents rather than private-state mutation;
- UI and input modalities invoke the same semantic command for shared operations;
- major layer dependency graph remains acyclic;
- contracts/interfaces live beside consumers instead of expanding a generic coordinator-types hub;
- production-path tests prove the new boundary is actually used, not merely instantiated.

## Evidence ladder

A tranche is not complete because a class exists or a unit test can instantiate it.

For each RF-061 tranche capture:

1. **Dependency evidence** — the prohibited edges have disappeared and a negative fixture proves the policy would catch their return.
2. **Production-path behavior evidence** — the real product action traverses the new seam and produces the same governed observable outcome.
...
5. **Adversarial post-review** — inspect the branch for a replacement god class, service locator, unused abstraction, duplicate path or tests that only prove existence.

USIM-A should be reused where it can falsify XR lifecycle, presentation-independent restore/replay and resource-balance regressions. Physical Quest remains authoritative for device performance/comfort and is not an RF-061 architecture-completion requirement unless a tranche changes device-dependent behavior.
...
This is evolutionary guidance, not a mass-move requirement:
...
Let the structure emerge as ownership moves; do not reorganize directories merely to resemble this diagram.

## Programme exit gate

RF-061 becomes `VERIFIED COMPLETE` only when:

- `World.ts` remains the explicit Nemosyne runtime composition root and public compatibility facade but no longer acts as authoritative owner of analytical, session, input, networking or representation state;
...
- architecture-policy checks prevent the dependency shape from regressing;
- production-path tests prove each migrated seam is used;
