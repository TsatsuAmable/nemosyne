...
The columnar profile follow-up closes that ABI discontinuity without reopening migration authority: the row-free Rust profile matches the row-backed contract and transfers approximately 2.7 KB at 10M. It also exposes the next performance limit—approximately 10.7 seconds for cold fingerprinting, 3.2 seconds for evidence generation and 1.25 GB of retained WASM memory on the local development baseline. These are product/device blockers tracked in `docs/ROADMAP.md`, not a reversal of the completed authority migration.
